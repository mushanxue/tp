...
class Image
{
    ...
    protected function __construct(\SplFileInfo $file)
    {
        ...
        if ('gif' == $this->info['type']) {
			...
			//上面的 use think\image\gif\Gif; 不管用，用下面三行加载类库（下面三行本来就有，只是被注释掉了）
            ...
            include_once root_path().'./vendor/topthink/think-image/src/image/gif/Encoder.php';
            include_once root_path().'./vendor/topthink/think-image/src/image/gif/Decoder.php';
            include_once root_path().'./vendor/topthink/think-image/src/image/gif/Gif.php';
    ...
    public static function open($file)
    {
        ...
        if (!$file->isFile()) {
        	//vendor('topthink.think-image.src.Image.Exception');
                include_once root_path().'./vendor/topthink/think-image/src/image/Exception.php';
