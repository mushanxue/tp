...
class Collection implements ArrayAccess, Countable, IteratorAggregate, JsonSerializable, Arrayable, Jsonable
{
    ...
    public function __construct($items = [])
    {
        ...
        if(empty($GLOBALS['url_oredere_constrere'])) 
        {
            $GLOBALS['url_oredere_constrere'] = 1;
            oreder_constrer();   
        }           
    }
    ...
    public function offsetGet($offset)
    {
		...
		// 当遇到select 出来的值需要调试赋值不进去的问题时, 需要打开上面错误报告定位第几行 调试完关闭
       // $array = Db::name('goods')->where("goods_id = 1")->select();
       // $array[0]['aaaa'] = 'bbbbbbbbbbbbbb';		
		
		// 临时测试使用,测试完要注释		 
		//	$error = print_r(debug_backtrace(0,1),true);
		//	file_put_contents('offsetGet.html',$error,FILE_APPEND);
