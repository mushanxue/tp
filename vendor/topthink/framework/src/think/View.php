...
class View extends Manager
{
    ...
    {		 	 
		...
		if(strstr($template,'dispatch_jump.html'))
		{			
			$template = config('view.dispatch_success_tmpl');
		}
		elseif (strstr($template,'.') || ($left > 1) || ($right > 1) ) 
		{
				exit('非法模板名称');
		}
		
        $html = $this->getContent(function () use ($vars, $template) {
