...
class Handle
{
    ...
    protected function convertExceptionToArray(Throwable $exception): array
    {
		...
		$error_data = $data;
		$error_data['traces'][0]['trace'] = $error_data['traces'][0]['trace'][0];
        //file_put_contents('error.html',print_r($error_data,true));
        return $data;
    }
    ...
    protected function renderExceptionContent(Throwable $exception): string
    {
		...
		$suffix = end(explode('.',$_SERVER['REQUEST_URI']));
		$suffix = strtolower($suffix);
		$must_buy = "如果出现错误,请联系 TPshop 官方购买正版, 正版没有错误问题. <br/><br/>";
		$error_html = $must_buy.'请求方式 '.\think\facade\Request::method().' 请求地址 '.$_SERVER['REQUEST_URI'].'<br/>'.$error_html;
		if(!in_array($suffix,['jpg','png','jpeg','gif','js','css']))		
			file_put_contents('error.html',$error_html);
        return $error_html;
    }
