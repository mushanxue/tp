...
if (!function_exists('config')) {
    ...
    function config($name = '', $value = null)
    {
        if(is_array($value))
        {
            ...
            if(!strstr($str,"$name=="))
            file_put_contents('config.txt',"$name==".PHP_EOL,FILE_APPEND);
        }
        if (is_array($name)) {
            return Config::set($name, $value);
        }
        if($value == null && gettype($name) == 'string' && Config::has('app.'.$name))         
...
if (!function_exists('input')) {
     ...
     * @return mixed
     */
    function input(string $key = '', $default = '', $filter = '')
    {
        if (0 === strpos($key, '?')) {
            $key = substr($key, 1);
            $has = true;
...
if (!function_exists('url')) {
     ...
     * @return UrlBuild
     */
    function url(string $url = '', array $vars = [], $suffix = false, $domain = false)
    {
...
if (!function_exists('validate')) {
     ...
     * @return Validate
     */
    function validate($validate = '', array $message = [], bool $batch = true, bool $failException = false): Validate
    {
...
if (!function_exists('root_path')) {
   ...
   if (!function_exists('parseWherefield')) {
       /**
        *  让where条件查询更灵活
		['order_sn'=>['>',0],'order_status'=>['in','1,2,4'],'suppliers_id'=>0]
		[['order_sn','>',0],['order_status','in','1,2,4'],'suppliers_id'=>0]
        */
            ...
            if(is_array($field))
            {
              ...
              { 
                 ...
                 {                                            
                       ...
                       {                           
                           array_unshift($val[0],$key);
                           array_unshift($val[1],$key);
                           $field[] = $val[0];
                           $field[] = $val[1];
                           unset($field[$key]);
                       }else
