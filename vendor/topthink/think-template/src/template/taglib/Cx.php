...
class Cx extends Taglib
{
	...
	public function tagTpshop($tag, $content)
        {
            ...
            preg_match_all('/(\$[a-zA-Z0-9_]+\[\'[a-zA-Z0-9_]+\'\])/U', $sql, $matches); // 将 $goods['goods_id'] 换成 {$goods['goods_id']}
            ...
            $parseStr   =   '<?php
                                ...
                                $md5_key = md5("'.$sql.'");
                                $'.$result_name.' = $'.$name.' = cache("sql_".$md5_key);
                                if(empty($'.$name.'))
                                {                            
                                    $'.$result_name.' = $'.$name.' = \think\facade\Db::query("'.$sql.'"); 
                                    cache("sql_".$md5_key,$'.$name.','.$t.');
                                }    
                             ';  
	...
	public function tagAdv($tag, $content)
        ...
        $str = '<?php ';
        $str .= '$pid ='.$pid.';';
        $str .= '$ad_position = \think\facade\Db::name("ad_position")->column("position_id,position_name,ad_width,ad_height","position_id");';
        $str .= '$result = \think\facade\Db::name("ad")->where("pid=$pid  and enabled = 1 and start_time < '.strtotime(date('Y-m-d H:00:00')).' and end_time > '.strtotime(date('Y-m-d H:00:00')).' ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("'.$limit.'")->select();';
        $str .= '
...
foreach($result as $'.$key.'=>$'.$item.'):       
    ...
    if(input("get.edit_ad") && $'.$item.'["not_adv"] == 0 )
    {
