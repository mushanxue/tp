<?php /*a:3:{s:38:"../template/pc/rainbow/cart/cart2.html";i:1593659405;s:46:"../template/pc/rainbow/public/sign-header.html";i:1593659405;s:41:"../template/pc/rainbow/public/footer.html";i:1593659405;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    ...
    <link rel="stylesheet" type="text/css" href="/pc/rainbow/static/css/jh.css"/>
    <link rel="stylesheet" type="text/css" href="/pc/rainbow/static/css/jquery.datetimepicker.css"/>
    <script src="/pc/rainbow/static/js/jquery-1.11.3.min.js" type="text/javascript" charset="utf-8"></script>
    <script src="/pc/rainbow/static/js/jquery.datetimepicker.full.js" type="text/javascript" charset="utf-8"></script>
...
<form name="cart2_form" id="cart2_form" method="post" autocomplete="off">
    <!--下单的终端设备-->
    <input type="hidden" name="from_terminal" value="pc">
    ...
    <!--立即购买才会用到-s-->
    <input type="hidden" name="action" value="<?php echo app('request')->param('action'); ?>">
    <input type="hidden" name="goods_id" value="<?php echo app('request')->param('goods_id'); ?>">
    <input type="hidden" name="item_id" value="<?php echo app('request')->param('item_id'); ?>">
    <input type="hidden" name="goods_num" value="<?php echo app('request')->param('goods_num'); ?>">
    <input type="hidden" name="auth_code" value="<?php echo config('AUTH_CODE'); ?>"/>
    <!--立即购买才会用到-e-->
    ...
    <input type="hidden" name="shopping_card_ids" value="">
</form>
...
<div class="sendgoodslist z-parkage-cont">
    <div class="w1224">
        <div class="top_leg p ma-to-20">
            ...
            <?php if(app('request')->param('action') != 'buy_now'): ?>
                <a class="newadd fr" href="<?php echo url('Home/Cart/index'); ?>">返回修改购物车</a>
            <?php endif; ?>
                        ...
                        <?php if(is_array($cartList) || $cartList instanceof \think\Collection || $cartList instanceof \think\Paginator): $i = 0; $__LIST__ = $cartList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cart): $mod = ($i % 2 );++$i;?>
                            <li>
                                <div class="goods-extra clearfix">
                                    <div class="p-img">
                                        <a target="_blank"
                                           href="<?php echo url('Home/Goods/goodsInfo',array('id'=>$cart['goods_id'])); ?>">
                                    ...
                                    <div class="goods-msg clearfix">
                                        <div class="goods-msg-gel">
                                            <div class="tp-cart-goods-name">
                                                <a href="<?php echo url('Home/Goods/goodsInfo',array('id'=>$cart['goods_id'])); ?>"
...
<div class="usecou-step-tit" id="usecou-step-tit">
    <div class="w1224">
        <div class="top_leg p ma-to-20">
            <span class="paragraph usewhilejs fl"><i class="ddd"></i>使用优惠券/购物卡/抵用</span>
        ...
        <div class="coupon-detail">
            <div class="detail-title clearfix">
                <ul class="available-title">
                    <li class="li active"><a href="javascript:;">优惠券 </a></li>
                    <?php if(count($cartList) == 1): ?>
                        <li class="li"><a href="javascript:;">购物卡</a></li>
                    <?php else: ?>
                        <li class="use_no"><a href="javascript:;">购物卡不可用</a></li>
                    <?php endif; ?>
                </ul>
                <?php if(count($cartList) != 1): ?>
                    <span class="tishi">购物卡只能用于单个商品</span>
                <?php endif; ?>
            </div>

            <div class="detail-cont coupons">
                <ul class="user">
                    <li class="user_t">可用( <i class="available-num">0</i> )</li>
                    <li>不可用( <i class="unavailable-num">2</i> )</li>
                </ul>
                <ul class="available coupons_card">
                    <li>
                        <div class="coupon-list coupon-able-list p">
                            <?php if(is_array($userCartCouponList) || $userCartCouponList instanceof \think\Collection || $userCartCouponList instanceof \think\Paginator): $i = 0; $__LIST__ = $userCartCouponList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$userCoupon): $mod = ($i % 2 );++$i;if($userCoupon['coupon']['able'] == 1): ?>
                                    <div class="coupon-item" data-date="<?php echo $userCoupon['coupon']['is_expiring']; ?>"
                                        ...
                                        <p class="time-over">
                                            有效期:<?php echo date('Y.m.d',!is_numeric($userCoupon['coupon']['use_start_time'])? strtotime($userCoupon['coupon']['use_start_time']) : $userCoupon['coupon']['use_start_time']); ?>-<?php echo date('Y.m.d',!is_numeric($userCoupon['coupon']['use_end_time'])? strtotime($userCoupon['coupon']['use_end_time']) : $userCoupon['coupon']['use_end_time']); ?></p>
                ...
                <div class="detail-cont card">
                <ul class="user">
                    <li class="user_t">可用( <i class=""><?php echo count($usableCard); ?></i> )</li>
                    <li>不可用( <i class=""><?php echo count($unsableCard); ?></i> )</li>
                </ul>
                <ul class="available_card coupons_card">

                    <li class="card_t">
                        <?php if(is_array($usableCard) || $usableCard instanceof \think\Collection || $usableCard instanceof \think\Paginator): $i = 0; $__LIST__ = $usableCard;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                            <div class="card-item" data-id=<?php echo $vo['id']; ?>>
                                <div class="img">
                                    <img src="<?php echo (isset($vo['shopping_card']['back_img']) && ($vo['shopping_card']['back_img'] !== '')?$vo['shopping_card']['back_img']:"/public/images/icon_goods_thumb_empty_300.png"); ?>" alt="">
                                </div>
                                <div class="card_info">
                                    <p><?php echo $vo['shopping_card']['name']; ?></p>
                                    <p>余额：￥<?php echo $vo['balance']; ?></p>
                                </div>
                                <i class="checked-ico"></i>
                            </div>
                        <?php endforeach; endif; else: echo "" ;endif; ?>

                        <p class="coupon-list-des"><i class="ico-warn"></i>购物卡可叠加使用</p>
                    </li>
                    <li>
                        <?php if(is_array($unsableCard) || $unsableCard instanceof \think\Collection || $unsableCard instanceof \think\Paginator): $i = 0; $__LIST__ = $unsableCard;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                        <div class="card-item">
                            <div class="img">
                                <img src="<?php echo (isset($vo['shopping_card']['back_img']) && ($vo['shopping_card']['back_img'] !== '')?$vo['shopping_card']['back_img']:"/public/images/icon_goods_thumb_empty_300.png"); ?>" alt="">
                            </div>
                ...
                <p class="item">
                    <label>
                        ...
                        <?php if(empty($user['paypwd'])): ?>
                            请先<a href="<?php echo url('User/paypwd'); ?>" style="color: #e23435;">设置支付密码</a>
                            <?php else: ?>
                            <a href="<?php echo url('User/paypwd'); ?>" target="view_window" style="color: #005ea7;">忘记支付密码？</a>
                        <?php endif; ?>
                    </label>
                </p>
            </div>
...
//            优惠券/购物卡
            $_couponWrap.find('.available-title .li').click(function () {
                ...
                if(index==0){
                    $_couponWrap.find('.detail-cont.card').hide();
                    $_couponWrap.find('.detail-cont.coupons').show();
                }else{
                    $_couponWrap.find('.detail-cont.coupons').hide();
                    $_couponWrap.find('.detail-cont.card').show();
                }
            })
//            选择购物卡
            $(".available_card .card_t .card-item").click(function(){
                if($(this).hasClass("checked")){
                    $(this).removeClass("checked");
                }else{
...
//                获取所有选择的购物卡id
                var card_id=[];
                $(".card-item.checked").each(function(){
                    card_id.push($(this).attr("data-id"));
                })
                var card_all=card_id[0];
                for(var i=0;i<card_id.length;i++){
                    if(i>0){
                        card_all=card_all+'_'+card_id[i]
                    }
                }
                if(card_id.length==0){
                    $("input[name='shopping_card_ids']").attr('value','');
                    ajax_order_price();
                    return false;
                }
                $("input[name='shopping_card_ids']").attr('value',card_all);
                ajax_order_price();
            })
//            可用/不可用
            $_couponWrap.find('.detail-cont .user li').click(function () {
                $(this).addClass('user_t').siblings().removeClass('user_t');
                $(this).parent().parent().find('.coupons_card li').eq($(this).index()).show().siblings().hide();
            })
//            $_couponWrap.find('.available-title li').click(function () {
//                $(this).addClass('active').siblings().removeClass('active');
...
<div class="order-summary p">
    <div class="w1224">
        <div class="statistic fr">
            ...
            <div class="list">
                <span>购物卡抵扣：</span>
                <em class="price" id="cardFee"> -￥0.00</em>
            </div>
            <div class="list">
                <span>优惠：</span>
...
<script type="text/javascript">
        ...
        $(document).on("click", '#coupon_exchange', function (e) {
            if ($('#coupon_code_checkbox').is(':checked')) {
                ...
                if (coupon_code != '') {
                    $.ajax({
                        type: "POST",
                        url: "<?php echo url('Home/Cart/cartCouponExchange'); ?>",
                        dataType: 'json',
    ...
    function ajax_address() {
        $.ajax({
            url: "<?php echo url('Home/Cart/ajaxAddress'); ?>", //+tab,
            success: function (data) {
    ...
    function ajax_order_price() {
        $.ajax({
            type: "POST",
            url: "<?php echo url('Home/Cart/cart3'); ?>",
            dataType: 'json',
            ...
            success: function (data) {
                ...
                if (data.status != 1) {
                    layer.msg(data.msg, {icon: 2, time: 2000},function(){
                        // 登录超时
                        if (data.status == -100) {
                            location.href = "<?php echo url('Home/User/login'); ?>";
                        }
                    });
                    // 显示商品有货无货配送
                ...
                if (data.result.card_price == null) {
                    $("#cardFee").text('-￥0');// 优惠券
                } else {
                    $("#cardFee").text('-￥' + data.result.card_price);// 优惠券
                }
    ...
    function submit_order() {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo url('Home/Cart/cart3'); ?>", //+tab,
    ...
    function get_shop_list(province_id, city_id, district_id, shop_address, longitude, latitude) {
        $.ajax({
            type: "POST",
            url: "<?php echo url('Home/Api/shop'); ?>",
            dataType: 'json',
            data: {
    ...
    $(function () {
        ...
        $(document).on("click", '.address_item', function (e) {
            ...
            if (address_id > 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo url('Home/User/address'); ?>",
        ...
        $(document).on("click", '#address_submit', function (e) {
            $.ajax({
                type: "POST",
                url: "<?php echo url('Home/User/addressSave'); ?>",
        ...
        $(document).on("click", '.address_set_default', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo url('Home/User/addressSetDefault'); ?>",
                data: {address_id: address_id},
                dataType: 'json',
                success: function (data) {
                    if (data.status == 1) {
        ...
        $(document).on("click", '.address_delete', function (e) {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo url('Home/User/addressDelete'); ?>",
                data: {address_id: address_id},
                dataType: 'json',
                success: function (data) {
                    if (data.status == 1) {
    ...
    function get_province() {
        $.ajax({
            type: "GET",
            url: "<?php echo url('Home/Api/getProvince'); ?>",
            dataType: 'json',
            success: function (data) {
                if (data.status == 1) {
...
<script type="text/javascript">
    ...
    function self_motion_load() {
        $.get("<?php echo url('Cart/invoice'); ?>", function (json) {
            var data = eval("(" + json + ")");
            if (data.status > 0) {
    ...
    function save_invoice() {
        ...
        $.post("<?php echo url('Cart/save_invoice'); ?>", data, function (json) {
            var data = eval("(" + json + ")");
            if(data.status==1){
