<?php /*a:4:{s:50:"../template/pc/rainbow/activity/pre_sell_list.html";i:1585277025;s:41:"../template/pc/rainbow/public/header.html";i:1593659405;s:41:"../template/pc/rainbow/public/footer.html";i:1593659405;s:47:"../template/pc/rainbow/public/sidebar_cart.html";i:1593659405;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<title>预售列表-<?php echo $tpshop_config['shop_info_store_title']; ?></title>
		...
		<script>
			...
			<?php if(is_array($pre_sell_list) || $pre_sell_list instanceof \think\Collection || $pre_sell_list instanceof \think\Paginator): if( count($pre_sell_list)==0 ) : echo "" ;else: foreach($pre_sell_list as $k=>$v): if($v['sell_end_time'] < time()): ?>
				...
				<?php elseif(($v['sell_end_time'] > time()) AND ($v['sell_start_time'] < time())): ?>
					var text<?php echo $v['goods_id']; ?>_<?php echo $v['item_id']; ?> = GetRTime('<?php echo date("Y/m/d H:i:s",!is_numeric($v['sell_end_time'])? strtotime($v['sell_end_time']) : $v['sell_end_time']); ?>');
					var goods_sy = "剩余";
					var goods_ys = "预售中";
				<?php else: ?>
					var text<?php echo $v['goods_id']; ?>_<?php echo $v['item_id']; ?> =  GetRTime('<?php echo date("Y/m/d H:i:s",!is_numeric($v['sell_start_time'])? strtotime($v['sell_start_time']) : $v['sell_start_time']); ?>');
					var goods_sy = "剩余";
					var goods_ys = "预热中";
				<?php endif; ?>
					$("#surplus_text<?php echo $v['goods_id']; ?>_<?php echo $v['item_id']; ?>").text(text<?php echo $v['goods_id']; ?>_<?php echo $v['item_id']; ?>);
					$("#sy_text_<?php echo $v['goods_id']; ?>_<?php echo $v['item_id']; ?>").text(goods_sy);
					$("#pre_sell_icons_<?php echo $v['goods_id']; ?>_<?php echo $v['item_id']; ?>").text(goods_ys);
			<?php endforeach; endif; else: echo "" ;endif; ?>
			}
							...
							<?php if(empty($pre_sell_list) || (($pre_sell_list instanceof \think\Collection || $pre_sell_list instanceof \think\Paginator ) && $pre_sell_list->isEmpty())): ?>
								...
								<?php else: if(is_array($pre_sell_list) || $pre_sell_list instanceof \think\Collection || $pre_sell_list instanceof \think\Paginator): $i = 0; $__LIST__ = $pre_sell_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
									<li class="pre_sell_li">
										<div id="pre_sell_icons_<?php echo $vo['goods_id']; ?>_<?php echo $vo['item_id']; ?>" class="pre_sell_icon"></div>
										<div class="s_xsall">
											<div class="xs_img">
												<a href="<?php echo url('Home/Goods/goodsInfo',array('id'=>$vo['goods_id'],'item_id'=>$vo['item_id'])); ?>">
													<img class="lazy-list" data-original="<?php echo goods_thum_images($vo['goods_id'],262,262); ?>" />
												</a>
											</div>
											...
											<div class="price-tag">
												...
												<span class="pre-per">已有<em><?php echo $vo['deposit_goods_num']; ?></em>人参与</span>
												<div class="sjs-olde p">
													<p class="djs-cctime olde-cont-wrap" ><em id="sy_text_<?php echo $vo['goods_id']; ?>_<?php echo $vo['item_id']; ?>" style="color: #333; font-size: 12px;margin-right: 3px;"></em><span id="surplus_text<?php echo $vo['goods_id']; ?>_<?php echo $vo['item_id']; ?>"></span></p>
												</div>
											</div>
										</div>
									</li>
