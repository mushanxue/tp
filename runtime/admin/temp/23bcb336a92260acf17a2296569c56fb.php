<?php /*a:2:{s:65:"Z:\TPshop20200701_tp6\app\admin\view\system/user_center_menu.html";i:1593659384;s:55:"Z:\TPshop20200701_tp6\app\admin\view\public/layout.html";i:1593659384;}*/ ?>
<!doctype html>
<html>
<head>
					...
					<ul>
						<?php if(is_array($menu_list) || $menu_list instanceof \think\Collection || $menu_list instanceof \think\Paginator): $i = 0; $__LIST__ = $menu_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?>
							<li data-menu-id="<?php echo $menu['menu_id']; ?>">
								...
								<div class="right_list_qk  cont_fl" style="display:none;">
									<div class="right_list_name cont_fl">
										切块
									</div>
	...
	<script type="text/javascript">
		...
		$(document).on("click","#submit",function  () {
			$.ajax({
				...
				data: $('#menu').serialize(),
				dataType: 'json',
				url: "<?php echo url('System/user_center_menu_save'); ?>",
				success: function (data) {
					if (data.status == 1) {
						location.reload();
					} else {
