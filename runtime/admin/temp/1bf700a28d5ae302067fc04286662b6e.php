<?php /*a:1:{s:43:"Z:\demo\app\admin\view\order/ajaxindex.html";i:1593659384;}*/ ?>
<table>
 	<tbody>
	...
	<?php else: if(is_array($orderList) || $orderList instanceof \think\Collection || $orderList instanceof \think\Paginator): $i = 0; $__LIST__ = $orderList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
  	<tr data-order-id="<?php echo $list['order_id']; ?>" id="<?php echo $list['order_id']; ?>">
        ...
        <td align="center" abbr="article_time" axis="col6" class="">
          <div style="text-align: center; width: 120px;" class=""><?php echo date('Y-m-d H:i',!is_numeric($list['add_time'])? strtotime($list['add_time']) : $list['add_time']); ?></div>
        </td>
        <td  axis="col1" align="center">
            <div style="text-align: left; width: 150px;"><?php echo $list['invoice_title']; ?></div>
        </td>
        <td  axis="col1" align="center">
            <div style="text-align: left; width: 150px;"><?php echo $order_from[$list['from_terminal']]; ?></div>
        </td>
		<td style="width: 100%;">
            <div style="text-align: left; ">
                <a class="btn green" href="<?php echo url('Admin/order/detail',array('order_id'=>$list['order_id'])); ?>">查看</a>
