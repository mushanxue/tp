...
class weixin{
	...
	public function __construct($config){
		...
		$this->return_url = SITE_URL.url('LoginApi/callback',array('oauth'=>'weixin'));
		
	}
	//构造要请求的参数数组，无需改动
	public function login(){
		if(isMobile()){
//			$url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$this->appid}&*redirect_uri=0yogbw3lcenc4skoyy7j9k61hlyd7bo7&response_type=code&scope=snsapi_userinfo&state=t1qsihde#wechat_redirec";
			$url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$this->appid}&redirect_uri=".urlencode($this->return_url)."&response_type=code&scope=snsapi_userinfo&state=t1qsihde#wechat_redirect";
			header("Location:$url");
			exit;
		}
