...
{    
    ...
    public function  __construct() { 
        ...
        $paymentPlugin = Db::name('Plugin')->where("code='tenpay' and  type = 'payment' ")->find(); // 找到支付插件的配置
    ...
    {                           
            ...
            $reqHandler->setParameter("notify_url", SITE_URL.url('Payment/notifyUrl',array('pay_code'=>'tenpay'))); //服务器异步通知页面路径 //必填，不能修改
            $reqHandler->setParameter("return_url",  SITE_URL.url('Payment/returnUrl',array('pay_code'=>'tenpay')));//页面跳转同步通知页面路径            
                ...
                if ($resHandler->getParameter("trade_mode") == "1") {
                    ...
                    if ($queryRes->isTenpaySign() && $queryRes->getParameter("retcode") == "0" && $resHandler->getParameter("trade_state") == "0") {
						...
						if (stripos($out_trade_no, 'recharge') !== false)
							$order_amount = Db::name('recharge')->where(['order_sn' => $out_trade_no, 'pay_status' => 0])->value('account');
						else						
