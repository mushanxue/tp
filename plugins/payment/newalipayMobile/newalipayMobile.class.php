...
class newalipayMobile extends Model
{
	...
	{            
		...
		$paymentPlugin = Db::name('Plugin')->where("code='newalipayMobile' and  type = 'payment' ")->find(); // 找到支付插件的配置
		...
		$this->alipay_config['notify_url']= SITE_URL.url('Payment/notifyUrl',array('pay_code'=>'newalipayMobile')); //异步通知地址
		$this->alipay_config['return_url']= SITE_URL.url('Payment/returnUrl',array('pay_code'=>'newalipayMobile')); //同步跳转
    ...
    function response()
    {
		...
		if($result) {//验证成功
			...
			if (stripos($order_sn, 'recharge') !== false)
				$order_amount = Db::name('recharge')->where(['order_sn' => $order_sn, 'pay_status' => 0])->value('account');
			else
