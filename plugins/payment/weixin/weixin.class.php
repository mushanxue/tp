...
class weixin
{
    ...
    function getJSAPI($order)
    {
        if(stripos($order['order_sn'],'recharge') !== false){
            $go_url = url('Mobile/User/recharge_list',array('type'=>'recharge'));
            if($order['level_id']>0){
                $back_url = url('Mobile/Distribut/become_distribut');
            }else{
                $back_url = url('Mobile/User/recharge',array('order_id'=>$order['order_id']));
            }
        }else{
            // $go_url = url('Mobile/Order/order_detail',array('id'=>$order['order_id']));
            $go_url = url('Mobile/Order/paySuccess',array('id'=>$order['order_id'])); 
