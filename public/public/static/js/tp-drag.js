    ...
    function img_call_back(fileurl_tmp){
        $('#tpd-mobile-views').children().each(function(){
            if($(this).hasClass('tpd-editing')){
                ...
                var that = $(this);
                setTimeout(function () {
                    if(that.children().find('#h-bgimg').length > 0
                        && typeof(that.children().find('#h-bgimg')[0].naturalWidth) != 'undefined'){
        ...
        // 悬浮自定义客服
        if(choice_type=='21'){
            console.log(gol,465464,fileurl_tmp,block[divid])
            //item=parseInt($(gol).parents('li').index());

            var li=$(gol).parents('li');
            var viewUl=$('.tpd-edits-hidden.tpd-editing').find('.tpd-kf-box');
            viewUl.find('img').attr('src',fileurl_tmp);
            var viewUl1=$('.tpd-diy-js.tpd-kf').find('.new_kf');
            viewUl1.find('img').attr('src',fileurl_tmp);
            // $(gol).html('<img style="width:80px;height:80px" src="'+fileurl_tmp+'">');
            $(gol).html('<div><img style="margin-left: 5px;width:90px !important;height:90px !important" src="'+fileurl_tmp+'"></div>');
            $(gol).append('  <label style="margin-left: 2px;">建议宽高<span id="sizeMsg">90*90</span>px<br /></label>');

            // var i=item-1;//最开始节点命名问题 没有从0开始
            block[divid].pic=fileurl_tmp;
...
if(info){
    ...
    block.is_kf=0;  //是否已存在客服悬窗 0不存在  1存在
...
function createModule(eidtid){
    ...
    if(eidtid==21 && is_aready('21')){
        layer.msg('只能添加一个客服悬浮窗', {icon: 2, time: 1000});
        return false;
    }
    ...
    */
    ...
    if(eidtid=='21'){
        block[divid].blank_bg_color = '#ffffff'
        block[divid].blank_range = '21'
        block[divid].kf_url_type = 0
        block.is_kf = 1;
        li_split1();
        // $('.tpd-blank .tpd-txtnav-text input[type=color]').val("#ffffff")
        // $('.tpd-blank .tpd-txtnav-text input[type=range]').val("20")
...
function li_split1(){
    ...
    var viewUl1=$('.tpd-edits-hidden.tpd-editing').find('.tpd-kf-box img').attr('src');
    block[divid].pic = viewUl1;
    $('.tpd-diy-js.tpd-kf').find('.new_kf img').attr('src',viewUl1);
    var i = block[divid].kf_url_type;
    $("#kf_url_type").find('label').eq(i).find('input').prop('checked',true)
}
//快捷入口编辑状态时数据还原
function li_split(){
    var li_str='';
        ...
        if (data.status == 1) {
            ...
            if (data.result.length > 0) {
                for (var i = 0; i < data.result.length; i++) {
                    ...
                    html += '<li><a class="tpdm-goods-pic" href="'+link_url+'"><img src="'+data.result[i].share_img+'" alt="" /></a>';
                    html += '<a href="'+link_url+'" class="tpdm-goods-name">'+data.result[i].goods_name+'</a>';
                    html += '<div class="tpdm-goods-des">';
                    html += '<div class="tpdm-goods-price">￥'+data.result[i].team_price+'</div>';
                    if(data.result[i].goods){
...
$(function(){
    ...
    $("body").on('click','.hand1',function(){
        ...
        if(block[divid].block_type == '13'){
            ...
            if(typeof block[divid].nav[i] != 'undefined' && typeof block[divid].nav[i].sql_where.goods != 'undefined'){
