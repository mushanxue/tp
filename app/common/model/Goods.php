...
class Goods extends Model
{
    ...
    public function getGoodsConfigAttr($value, $data)
    {
        $nowTime = time();
        $res = Db::name('ad_goods')->alias('g')->join('ad_goods_config c','c.ad_goods_id = g.ad_goods_id','left')
            ->where("g.enabled =1 AND g.end_time > {$nowTime} AND g.start_time <{$nowTime} and( c.goods_id = {$data['goods_id']} OR c.goods_id is null)")
            ->group('c.goods_id')
            ->order(['c.goods_id'=>'asc','g.orderby'=>'desc'])
            ->find();
        return  $res ;
    }
    ...
    {
        return $this->hasMany('GoodsAttr', 'goods_id' ,'goods_id')->join('goods_attribute b', '__PREFIX__goods_attr.attr_id = b.attr_id')->order('b.order desc');
    }
    ...
    {
//        $comment_where = ['is_show' => 1, 'goods_id' => $data['goods_id'], 'user_id' => ['>', 0]]; //公共条件
        $comment_where = ['is_show' => 1, 'goods_id' => $data['goods_id'], 'username' => ['<>', 'admin']]; //公共条件
...
//            "sum(case when goods_rank < 3 then 1 else 0 end) as low_sum,count(comment_id) as total_sum";
    ...
    {
        $spec_goods_price_key = Db::name('spec_goods_price')->where("goods_id", $data['goods_id'])->column('key');
        if($spec_goods_price_key){
            ...
            $spec_item_list = Db::name('spec_item')->where('id', 'IN', $spec_goods_price_key_arr)->order('order_index asc')->select();
            $spec_ids = get_arr_column($spec_item_list, 'spec_id');
            $spec_list = Db::name('spec')->where('id', 'IN', $spec_ids)->order(['order'=>'desc','id'=>'asc'])->select()->toArray();
    ...
    public function getOrderBespeakCountAttr($value, $data)
    {
        ...
        $count = Db::name('shop_order')
            ...
            ->where(['s.goods_id'=>$data['goods_id'],'o.pay_status'=>1,'o.order_status'=>['<>',3]])
     ...
     * @return int|string
     */
    public function  getQuestionTotalAttr($value,$data)
    {
        //是否登陆 本人可查看本人提交的未审核问题
        $login = session('user');
        if($login!=null){
        ...
        $count = Db::name("askall_question")->where(['goods_id' => $data['goods_id'], 'status' => 1])->where(function ($query) use($userId) {
            $query->where('audit', 1);
            if($userId > 0){//如果用户登录 查找我的未审核问题
                $query->whereOr(function ($query) use($userId) {
                    $query->where(['user_id'=>$userId, "audit"=>0]);
                });
            }
        })->count();
//        旧版本 不判断登录状态
//        $count = Db::name('askall_question')->where(['goods_id' => $data['goods_id'], 'audit' => 1, 'status' => 1])->count();
        return $count;
