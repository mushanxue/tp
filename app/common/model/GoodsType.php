...
class GoodsType extends Model {
    ...
    public function save($data = [], $sequence = null): bool
    {
        ...
        foreach($attribute as $attribute_item){
            if(array_key_exists('attr_id', $attribute_item) && $attribute_item['attr_id'] > 0){
                $goodsAttribute = GoodsAttribute::find($attribute_item['attr_id']);
            }else{
        ...
        if(count($attr_ids) > 0){
            Db::name('goods_attribute')->where(['type_id'=>$this->getAttr('id')])->where('attr_id','NOTIN', $attr_ids)->delete();
        }
        return $result;
    }
