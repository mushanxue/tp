...
class PromotionBargain extends Model {
    ...
    {
        $data['promotionBargainGoodsItem'] = Db::name('promotion_bargain_goods_item')->field('sum(goods_num) as goods_num , sum(buy_num) as buy_num')->where(['bargain_id'=>$data['id']])->find();
    ...
    public function getStatusDescAttr($value, $data)
    {
        if($data['is_end'] == 1){
            ...
            $promotion_bargain_goods_item =  Db::name('promotion_bargain_goods_item')->field('*,sum(buy_num) as buy_num ,sum(goods_num) as goods_num ')->where(['bargain_id'=>$data['id']])->find();
