...
class MessageActivity extends Model
{
    ...
    public function getHomeUrlAttr($value, $data)
    {
        switch ($data['mmt_code']) {
            case 'flash_sale_activity':
                // 抢购
                $flash_goods = Db::name('flash_goods')->where('flash_sale_id', $data['prom_id'])->find();
                $uri = U("Home/Goods/goodsInfo", ['id' => $flash_goods['goods_id'], 'item_id' => $flash_goods['item_id']]);
                break;
            case 'group_buy_activity':
                // 团购
                $flash_sale = Db::name('group_buy')->where('id', $data['prom_id'])->find(); 
                $uri = url("Home/Goods/goodsInfo", ['id' => $flash_sale['goods_id'], 'item_id' => $flash_sale['item_id']]);
                break;
            case 'prom_goods_activity':
            case 'prom_order_activity':
                // 优惠促销 列表
                $uri = url("Home/Activity/promoteList");
                break;
            case 'combination_activity':
                // 搭配购 主商品
                $goods_id = Db::name('combination_goods')->where(['combination_id' => $data['prom_id'], 'is_master' => 1])->value('goods_id'); 
                $uri = url("Home/Goods/goodsInfo", ['id' => $goods_id]);
                break;
            case 'team_activity':
                // 拼团 只有手机有 Mobile/Goods/goodsInfo/id/309/item_id/0
                $flash_sale = Db::name('team_activity')->where('team_id', $data['prom_id'])->find(); 
                $uri = url("Mobile/Goods/goodsInfo", ['id' => $flash_sale['goods_id'], 'item_id' => $flash_sale['item_id']]);
                break;
            default:
                $uri = '';
                break;
    ...
    public function getMobileUrlAttr($value, $data)
    {
        if ($data['mmt_code'] == 'flash_sale_activity') {
            // 抢购
            $flash_sale = Db::name('flash_goods')->where('flash_sale_id', $data['prom_id'])->find();
            $uri = url("Mobile/Goods/goodsInfo", ['id' => $flash_sale['goods_id'], 'item_id' => $flash_sale['item_id']]);
            ...
            // 优惠促销 列表
            $uri = url("Mobile/Activity/promote_goods");
        } elseif ($data['mmt_code'] == 'combination_activity'){
            // 搭配购 主商品
            $goods_id = Db::name('combination_goods')->where(['combination_id' => $data['prom_id'], 'is_master' => 1])->value('goods_id'); 
            $uri = url("Mobile/Goods/goodsInfo", ['id' => $goods_id]);
            ...
            // 砍价
            $goods_id = Db::name('promotion_bargain')->alias('p')->join('promotion_bargain_goods_item g','p.id = g.bargain_id')->where(['id' => $data['prom_id']])->field('g.goods_id,item_id')->find();
            $uri = url("Mobile/Goods/goodsInfo", ['id' => $goods_id['goods_id'],'item_id'=>$goods_id['item_id']]);
        } else {
            $uri = '';
        }
        return $uri;
