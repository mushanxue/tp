...
class Users extends Model
{
    ...
    public function getUserTeamOrderAttr($value, $data){
        $first_leader = DB::name('users')->where(['first_leader'=>$data['user_id']])->column('user_id');
        $second_leader = DB::name('users')->where(['second_leader'=>$data['user_id']])->column('user_id');
        $third_leader = DB::name('users')->where(['third_leader'=>$data['user_id']])->column('user_id');
    ...
    public function getUserOrderAttr($value, $data){
        /*
        $order = DB::name('rebate_log')->alias('a')->join('tp_order b','a.order_id=b.order_id','left')->where(['a.user_id'=>$data['user_id'] ,'a.status'=> [ 'in',[1,2,3]] ])->field('b.order_id,b.total_amount')->select();
        $count = 0;
        $total_amount = 0;
        if($order){
            foreach ($order as $v){
