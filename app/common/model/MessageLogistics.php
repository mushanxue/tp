...
class MessageLogistics extends Model
{
    ...
    public function getHomeUrlAttr($value, $data)
    {
        if ($data['mmt_code'] == 'evaluate_logistics'){
            $uri = url("Home/Order/comment", ['status' => 0]);
        }else{
            ...
            if($prom_type == 6){
                ...
                $uri = url("Home/Order/order_detail", ['id' => $data['order_id']]);
            }
        }
        return $uri;
    }
    ...
    {
        if ($data['mmt_code'] == 'evaluate_logistics'){
            $uri = url("Mobile/Order/comment", ['status' => 0]);
        }else{
            $prom_type = Db::name('order')->where('order_id', $data['order_id'])->value('prom_type');
            if($prom_type == 6){
                // 0普通订单4预售订单5虚拟订单6拼团订单',
                $uri = url("Mobile/Order/team_detail", ['order_id' => $data['order_id']]);
            }elseif($prom_type == 5){
                $uri = url("Mobile/Virtual/virtual_order", ['order_id' => $data['order_id']]);
            }else{
                $uri = url("Mobile/Order/order_detail", ['id' => $data['order_id']]);
            }
        }
        return $uri;
    } 
