...
class User
{
    ...
    {
        $this->user = Users::where(['mobile' => $mobile])->find();
    }
    ...
    public function checkOauthBind()
    {
        ...
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false && ACTION_NAME != 'bind_account') {
            //微信小程序访问
            if(!cache(input('unique_id'))  && !session('third_oauth')){
                throw new TpshopException('关联账号', 0, ['status' => 0, 'msg' => '登录超时']);
            }
            if(cache(input('unique_id'))){
                session('third_oauth',cache(input('unique_id')));
            }
        }
    ...
    {
        /* $first_leader = cookie('first_leader')?: cache(input('unique_id').'first_leader');  //推荐人id
        ...
        if ($first_leader && $first_leader != $this->user->first_leader) {
            ...
            $this->user->save();
            $firstLeaderUser = Users::where(['user_id' => $first_leader])->find();
            if ($firstLeaderUser) {
                //他上线分销的下线人数要加1
                $firstLeaderUser->underling_number = $firstLeaderUser->underling_number + 1;
                $firstLeaderUser->save();
                Db::name('users')->where(['user_id' => $firstLeaderUser['second_leader']])->inc('underling_number')->update();
                Db::name('users')->where(['user_id' => $firstLeaderUser['third_leader']])->inc('underling_number')->update();
            }
        } else {
    ...
    {
        $total_amount = Db::name('order')->master(true)->where(['user_id' => $this->user['user_id'], 'pay_status' => 1, 'order_status' => ['NOTIN', [3, 5]]])->sum(Db::raw('order_amount+user_money'));
        $level_info = Db::name('user_level')->where([['amount' , '<=', $total_amount]])->order('amount desc')->find();
