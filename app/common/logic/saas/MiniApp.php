...
class MiniApp
{
    ...
    public function commitTemplate()
    {
        ...
        //只能有一个体验版
        $user_miniApp = UserMiniapp::where(['user_id' => $this->miniApp['user_id'], 'miniapp_id' => $this->miniApp['miniapp_id'], 'status' => UserMiniapp::STATUS_TEST])->find();
        if ($user_miniApp) {
    ...
    {
        $test_mini_app = UserMiniapp::where(['user_id' => $this->miniApp['user_id'], 'miniapp_id' => $this->miniApp['miniapp_id'], 'status' => UserMiniapp::STATUS_TEST])->find();
        if (empty($test_mini_app)) {
            throw new TpshopException('小程序提交审核', -1 ,['status'=>-1, 'msg'=>'体验版本不存在，不能提交审核']);
        }
        $audit_mini_app = UserMiniapp::where(['miniapp_id' => $this->miniApp['miniapp_id'], ['status' , 'in', [UserMiniapp::STATUS_AUDITING, UserMiniapp::STATUS_AUDIT_DONG]]])->find();
        if ($audit_mini_app) {
