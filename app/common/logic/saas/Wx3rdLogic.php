...
class Wx3rdLogic
{
    ...
    private function handleAuthSuccessEvent($msg)
    {
        ...
        $miniapp && $miniapp = Miniapp::where(['miniapp_id' => $miniapp->miniapp_id])->find();
        if ($miniapp && $user = Users::where(['user_id' => $miniapp->user_id])->find()) {
    ...
    {
        $miniapp = Miniapp::where(['origin_id' => $msg['ToUserName']])->find();
        if ($miniapp) {
