...
class BargainLogic
{
    ...
    public function setBargainId($bargain_id){
        ...
        $this->bargain = PromotionBargain::where(['id'=>$bargain_id,'is_end'=>0])->find();
    }
    ...
    public function setItemId($item_id){
        $this->promotion_bargain_goods_item = PromotionBargainGoodsItem::where(['bargain_id'=>$this->bargain_id,'item_id'=>$item_id])->find();
    }
    ...
    public function bargainList($data)
    {
        ...
        $count = $PromotionBargain->alias('b')
                ->field('b.*, sum(p.goods_num) - sum(p.buy_num) as remain_num  '.$field)
                ->where($where)
                ->join('promotion_bargain_goods_item p','p.bargain_id=b.id','left')
                ->join('goods g','g.goods_id=b.goods_id','left')
                ->order($sort)->group('id')->count();
            
        $Page = new Page($count, 10);
        $PromotionBargain = new PromotionBargain();
        $PromotionBargain = $PromotionBargain->alias('b')
                ->field('b.*,p.item_id,p.end_price, sum(p.goods_num)-sum(p.buy_num) as remain_num  '.$field)
                ->where($where)->join('promotion_bargain_goods_item p','p.bargain_id=b.id','left')
                ->join('goods g','g.goods_id=b.goods_id','left')->order($sort)
                ->group('id')->limit($Page->firstRow,$Page->listRows)->select();
        foreach ($PromotionBargain as $k=>$v)
        {
            $BargainFirst = BargainFirst::where(['bargain_id'=>$v['id'],'user_id'=>$this->user_id,'order_id'=>0])->find();
            ...
            if($v['item_id'] > 0){
                $PromotionBargain[$k]['shop_price'] = Db::name('spec_goods_price')->where(['item_id'=>$v['item_id']])->value('price');
            }else{
                $PromotionBargain[$k]['shop_price'] = Db::name('goods')->where(['goods_id'=>$v['goods_id']])->value('shop_price');
            }
        }
...
//                $where['o.pay_status'] = 0;
        ...
        $count = $BargainFirst
            ...
            ->join('promotion_bargain p','p.id=f.bargain_id','left')
            ->where($where)
            ->where($where4)
...
//fetchSql(true)->
       foreach ($BargainFirst as $k=>$v)
       {
           ...
           if(($v['order_addtime'] + $v['promotionBargain']['order_overtime']*60 < time() || $v['promotionBargain']['is_end'] == 1) && $v['pay_status'] === 0 && $v['order_status'] === 0 ){
               ...
               if($v['user_money'] > 0){
                   ...
                   $refundLogic->updateRefundOrder(Order::where(['order_id'=>$v['order_id']]),0)->find();
    ...
    {
        $this->bargainFirst = BargainFirst::where(['bargain_id'=>$this->bargain_id,'user_id'=>$this->user_id,'order_id'=>0])->find();
        if(!$this->bargainFirst){
            ...
            if($BargainFirst->save()){
                ...
                $this->cut(Users::find($this->user_id));
    ...
    public function showBargain()
    {
        ...
        $info['count_cut_price'] = Db::name('BargainList')->where(['bargain_first_id'=>$this->bargain_first_id])->sum('cut_price');
        $info['user_cut_price'] = Db::name('BargainList')->where(['bargain_first_id'=>$this->bargain_first_id,'user_id'=>$this->user_id])->find();
        if($this->checkActivityIsEnd()){
    ...
    public function getBargainFirst($user_id = 0)
    {
        ...
        $data =  BargainFirst::where($where)->find();
        if($data){
    ...
    {
        $data =  PromotionBargain::where(['id'=>$this->bargain_id])->find();
        if($data){
            $PromotionBargainGoodsItem =  PromotionBargainGoodsItem::where(['bargain_id'=>$this->bargain_id,'item_id'=>$this->bargainFirst['item_id']])->find();
    ...
    public function cut($user){
        ...
        if($data){
            ...
            $PromotionBargainGoodsItem =  PromotionBargainGoodsItem::where(['bargain_id'=>$this->bargain['id'],'item_id'=>$data['item_id']])->find();
            $BargainList = new BargainList();
    ...
    public function getUserCutLimit()
    {
        ...
        $BargainFirst = $BargainFirst->alias('b')->join('order o','o.order_id = b.order_id','left')->where($orderWhere)->count();
        return $BargainFirst;
    }
