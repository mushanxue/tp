...
class PromotionBargainLogic extends Prom
{
    ...
    public function __construct($goods, $specGoodsPrice)
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，规格和活动是一对一
            $this->bargain = \app\common\model\PromotionBargain::find($specGoodsPrice['prom_id']);
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->bargain = \app\common\model\PromotionBargain::find($goods['prom_id']);
        }
...
//    }
    ...
    public function getActivityGoodsInfo(){
        ...
        $promotionBargainGoodsItem =  PromotionBargainGoodsItem::where(['item_id'=>$activityGoods['item_id'],'bargain_id'=>$this->bargain['id']])->find();
        ...
        $bargain_first = 0;
        if(session('user')){
            $bargain_first = Db::name('bargain_first')->where(['user_id'=>session('user.user_id'),'bargain_id'=>$this->bargain['id']])->find();
        }
        $activityGoods['bargain_first'] = $bargain_first?1:0;//是否已参与
//        $activityGoods['bargain_list'] = Db::name('bargain_list')->where(['bargain_first_id'=>$bargain_first['id']])->select();
        $activityGoods['bargain_list'] = Db::name('bargain_first')->alias('b')->field('u.head_pic')->join('users u','u.user_id = b.user_id')->where(['bargain_id'=>$this->bargain['id']])->order('id desc')->select();
...
//        }
    ...
    public function buyNow($buyGoods){
        ...
        $bargain_first  = BargainFirst::where(['bargain_id'=>$this->bargain['id'],'user_id'=>$this->user_id,'order_id'=>0])->find();
        $promotionBargainGoodsItem =  PromotionBargainGoodsItem::where(['item_id'=>$bargain_first['item_id'],'bargain_id'=>$this->bargain['id']])->find();
