...
class PlaceOrder
{
    ...
    public function addNormalOrder()
    {
       ...
       //Hook::listen('user_add_order', $this->order);//下单行为
		...
		// $this->deductionCard();//扣除购物卡 后面升级TP6 之后 这个功能才完善的,
...
//        $this->changUserPointMoney();//扣除用户积分余额
    ...
    public function checkBespeakTemplate($order_goods, $shops, $take_time, $template_unit_id=0)
    {
        ...
        if($template_unit_id){
            //支付的时候进来这里
            $template_id = Db::name('bespeak_template_unit')->where(['template_unit_id'=>$template_unit_id])->value('template_id');
        ...
        $count = $ShopOrder::hasWhere('Order',['Order.pay_status'=>1])
            ->where(['ShopOrder.goods_id'=>$goodsArr['goods_id'],'Order.order_status'=>['<>',2],'ShopOrder.shop_id'=>$shop['shop_id'],'ShopOrder.take_time'=>date('Y-m-d H:i:s',$this->take_time)])
    ...
    public function validateBespeakForm($form){
        ...
        $get_template_unit = $bespeak_template_unit::find($ids[0]);//获取模板id

        //查出改模板所有组件验证数据
        $get_template_unit = $bespeak_template_unit::where(['template_id'=>$get_template_unit['template_id'],'deleted'=>0])->select();
    ...
    {
        Db::name('bargain_first')->where(['bargain_id'=>$this->order['prom_id'],'user_id'=>$this->order['user_id'],'order_id'=>0])->update(['order_id'=>$this->order['order_id'],'is_end'=>1]);
    }
    ...
    private function addOrderGoods()
    {
        ...
        $payList = $this->pay->getPayList();
        $goods_ids = get_arr_column($payList,'goods_id');
        $goodsArr = Db::name('goods')->where('goods_id', 'IN', $goods_ids)->column('goods_id,cost_price,give_integral','goods_id');
        $orderGoodsAllData = [];
        //砍价可能商品价格为0，除数不能为0，所以会导致报错，
        if($this->order['goods_price']){
            $rate = ($this->order['user_money'] + $this->order['order_amount'] - $this->order['shipping_price']) / $this->order['goods_price']; // 按实付比例 算积分
        ...
        foreach ($payList as $payKey => $payItem) {
            ...
            if (!empty($payItem['spec_key'])) {
                ...
                $spec_goods_price = Db::name('spec_goods_price')->where(['goods_id'=>$payItem['goods_id'],'key'=>$payItem['spec_key']])->find();
			...
			$orderGoodsData['give_integral'] = ceil($orderGoodsData['give_integral']); // 这里积分会跑出-3333.3333333333 情况
			$orderGoodsData['give_integral'] = $orderGoodsData['give_integral'] < 0 ? 0 : $orderGoodsData['give_integral']; // 会跑出负数的情况
    ...
    public function deductionCoupon()
    {
        ...
        if($couponId > 0){
            ...
            if($userCoupon){
                ...
                $userCoupon->status =  1;
                $userCoupon->save();
                Db::name('coupon')->where('id', $userCoupon['cid'])->inc('use_num')->update();// 优惠券的使用数量加一
            }
        }
    }
    /*
    ...
    {
        $shopping_card = $this->pay->getShoppingCardBalance();
        $variation_data=[];
        ...
        foreach($shopping_card as $card){
            ...
            $card_item['balance']=$card['balance'];
            $data[] = $card_item;
            $item['user_id']=$this->order['user_id'];
            $item['order_id']=$this->order['order_id'];
            $item['add_time']=time();
            $item['varuatuib_price']=$card['variation'];
            $item['is_return']=0;
            $item['card_list_id']=$card['id'];
            $variation_data[]=$item;
        }
        Db::name('shopping_card_variation')->insertAll($variation_data);
    ...
    public function setFromTerminal($from_terminal)
    {
