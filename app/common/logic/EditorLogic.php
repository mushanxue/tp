...
class EditorLogic
{
    ...
    public function waterImage($img_path)
    {
        ...
        $image = \think\image\Image::open($img_path);
    ...
    public function saveUploadImage($file, $save_path)
    {
        ...
        //Koala.jpg====
        //C:\Windows\phpAAB8.tmp====
        
        if (in_array($save_path, $waterPaths) && tpCache('oss.oss_switch')) {
            //商品图片可选择存放在oss
            $object = UPLOAD_PATH.$new_path.md5(time().mt_rand(100,999999999)).'.'.pathinfo($file->getOriginalName(), PATHINFO_EXTENSION);          
            ...
            if ($upload_error) {
                ...
                $return_url = '/'.UPLOAD_PATH.$new_path.$savename;
