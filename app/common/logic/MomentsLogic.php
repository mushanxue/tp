...
class MomentsLogic
{
    ...
    public static function select_moments($data)
    {
        ...
        foreach ($select as $k => $v) {
            ...
            $getComment = DB::name('moments_comment')
                ->alias('c')
                ->where($where)
                ->bind($bind)
...
//                $getLike[$k]['user_id']=null;
    ...
    {
        $add = Db::name('moments')->insertGetId($data);
        if ($add) {
    ...
    public static function add_like($data)
    {
        ...
        if ($find) {
            ...
            Db::name('moments')->where($where)->dec('like_sum')->update();
        } else {
            $add = Db::name('moments_like')->insertGetId($data);
            Db::name('moments')->where($where)->inc('like_sum')->update();
        }
        if ($add) {
            return array('status' => 1, 'msg' => '操作成功', 'result' => '');
        }
...
//        $where['moments_id'] = array('in', $moments);
        ...
        $where['m.user_id'] = array('<>', $data['user_id']);
        $where['o.user_id'] = array('=', $data['user_id']);
        $like = Db::name('moments_like')
            ->alias('m')
            ->join('users u', 'u.user_id = m.user_id', 'LEFT')
            ->join('moments o', 'o.moments_id = m.moments_id', 'LEFT')
...
//            })
        ...
        $arr = array_merge($like->toArray(), $comment->toArray());
