...
class VirtualLogic 
{
    ...
    public function check_virtual_goods($goods_id,$item_id=0,$goods_num=0){ 
        ...
        if($goods['is_virtual'] == 1 && $goods['virtual_indate']>time() && $goods['store_count']>0){
            ...
            if($item_id>0){
                $specGoodsPrice = DB::name('SpecGoodsPrice')->where(array('item_id'=>$item_id))->cache(true,TPSHOP_CACHE_TIME)->find(); // 获取商品对应的规格价钱 库存 条码
    ...
    public function orderList($user_id, $type, $search_key)
    {
        ...
        //条件搜索
        if ($type) {
            $where .= config(strtoupper($type));
        }
        // 搜索订单 根据商品名称 或者 订单编号
        $search_key = trim($search_key);
        if ($search_key) {
            $where .= " and (order_sn like :search_key1 or order_id in (select order_id from `" . config('database.connections.mysql.prefix') . "order_goods` where goods_name like :search_key2) ) ";
    ...
    public function orderListApi($user_id, $type, $search_key)
    {
        ...
        if(!$order_list->isEmpty()){
            $order_list = $order_list->append(['virtual_order_button','order_status_detail'])->toArray();
        }
        return $order_list;
    }
    ...
    public function check_virtual_code($order){
        $vrOrders = Db::name('vr_order_code')->where(array('order_id'=>$order['order_id']))->select()->toArray();
