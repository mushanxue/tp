...
//        Db::name('message_logistics')->insert($this->message);
//        $message_id = Db::name('message_logistics')->getLastInsIDb::name();		
		$message_id = Db::name('message_logistics')->insertGetId($this->message);
    ...
    {
        $message_id = Db::name('message_logistics')->where(['order_id' => $prom_id, 'type' => $type])->column('message_id');
        if ($message_id) {
            Db::name('message_logistics')->where(['order_id' => $prom_id, 'prom_type' => $type])->delete();
            Db::name('user_message')->where([['message_id' , 'in',$message_id], 'category' => 2])->delete();
        }
    }
