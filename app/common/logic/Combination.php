...
class Combination
{
    ...
    {
        $combinationId_arr = is_array($this->combinationId) ? $this->combinationId : [$this->combinationId];                
        $combination_list = $this->combinationModel
            ->with(['combination_goods'])
            ->where(['is_on_sale' => 1, ['start_time' , '<', time()], ['end_time' , '>', time()], 'combination_id' => ['IN', $combinationId_arr]])
            ->select();
