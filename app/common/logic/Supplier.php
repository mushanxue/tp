...
class Supplier 
{
    ...
    public function log($content)
    {
        ...
        Db::name('supplier_log')->insert($log);
    ...
    public function autoTransfer($suppliers_id){
        ...
        {
        	$return_goods = DB::name('return_goods')->where("order_id = {$val['order_id']} and status not in (-2,5)")->select();
        	if(!$return_goods->isEmpty()) continue;//如果有售后申请未完成，则不结算
			$order_goods = DB::name('order_goods')->where(array('order_id' => $val['order_id']))->select();//订单商品
        ...
        if(!empty($order_id_arr)){
            $order_statis_id = DB::name('order_statis')->insertGetId($data); // 添加一笔结算统计
