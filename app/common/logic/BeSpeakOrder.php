...
class BeSpeakOrder
{
    ...
    public function writeOff()
    {
        ...
        $shop_order_count = Db::name('shopOrder')->where([['is_write_off','<>',1],'order_id'=>$order->order_id])->count();
        ...
        if($shop_order_count == count($this->shopOrder)){
            ...
            Db::name('order_goods')->where(['order_id'=>$order->order_id])->save(['is_send'=>1]);  //把订单状态改为已收货
        ...
        public function setInvalidRefund($value)
    {
        ...
        $order =  \app\common\model\Order::find($value['order_id']);
