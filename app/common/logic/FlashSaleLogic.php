...
use app\common\model\FlashGoods;
...
class FlashSaleLogic extends Prom
{
    ...
    public function __construct($goods, $specGoodsPrice)
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，规格和活动是一对一
            $this->flashSale = FlashSale::find($specGoodsPrice['prom_id']);
	    $this->flashGoods = FlashGoods::where(["flash_sale_id"=>$specGoodsPrice['prom_id'], 'item_id'=>$specGoodsPrice['item_id']])->find();
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->flashSale = FlashSale::find($goods['prom_id']);
	    $this->flashGoods = FlashGoods::where(["flash_sale_id"=>$goods['prom_id'],'goods_id'=>$goods['goods_id']])->find();
        }
        if ($this->flashSale) {
            ...
            if ($this->checkActivityIsEnd() && $this->flashSale['is_end'] == 0) {
                if($this->specGoodsPrice){
                    ...
                    $this->specGoodsPrice = SpecGoodsPrice::find($specGoodsPrice['item_id']);
                }else{
                    Db::name('goods')->where("goods_id", $this->flashGoods['goods_id'])->save(['prom_type' => 0, 'prom_id' => 0]);
                }
                
                $spec_count = db('spec_goods_price')->where('prom_id', $goods['prom_id'])->count('item_id');
                $goods_count = db('goods')->where('prom_id', $goods['prom_id'])->count('goods_id');
                if ($spec_count<=0 && $goods_count<=0) {
                    $this->flashSale->is_end = 1;
                    $this->flashSale->save();
                }
                unset($this->goods);
                $this->goods = Goods::find($goods['goods_id']);
            }
        }
    }
    ...
    public function checkActivityIsEnd(){
        ...
        if($this->flashGoods['buy_num'] >= $this->flashGoods['goods_num']){
            return true;
        }
    ...
    public function getUserFlashOrderGoodsNum($user_id){
        ...
        if ($order_id_arr) {
            $orderGoodsWhere = ['prom_id' => $this->flashSale['id'], 'prom_type' => 1,'item_id'=>$this->flashGoods['item_id'],'goods_id'=>$this->flashGoods['goods_id'], 'order_id' => ['in', implode(',', $order_id_arr)]];
    ...
    public function getUserFlashResidueGoodsNum($user_id){
        ...
        $residue_num = $this->flashGoods['goods_num'] - $this->flashGoods['buy_num']; //剩余库存
        //限购》已购
        $residue_buy_limit = $this->flashGoods['buy_limit'] - $purchase_num;
     ...
     * @return static
     */
    public function getFlashGoodsModel(){
    ...
    public function getActivityGoodsInfo(){
        ...
        $activityGoods['shop_price']        = $this->flashGoods['price'];
        $activityGoods['store_count']       = $this->flashGoods['goods_num'] - $this->flashGoods['buy_num'];
        $activityGoods['start_time']        = $this->flashSale['start_time'];
        $activityGoods['end_time']          = $this->flashSale['end_time'];
        $activityGoods['buy_limit']         = $this->flashGoods['buy_limit'];
    ...
    public function buyNow($buyGoods){
        ...
        if($this->getActivityGoodsNum() >= $this->flashGoods['goods_num']){
            throw new TpshopException('抢购商品立即购买', 0, ['status' => 0, 'msg' => '该抢购商品库存已锁定！', 'result' => '']);
        }
        if($this->checkActivityIsAble()){
            if($buyGoods['goods_num'] > $this->flashGoods['buy_limit']){
                throw new TpshopException('抢购商品立即购买', 0, ['status' => 0, 'msg' => '每人限购'.$this->flashGoods['buy_limit'].'件', 'result' => '']);
            }
        }
        ...
        if($userBuyGoodsNum > $this->flashGoods['buy_limit']){
            throw new TpshopException('抢购商品立即购买', 0, ['status' => 0, 'msg' => '每人限购'.$this->flashGoods['buy_limit'].'件，您已下单'.$userFlashOrderGoodsNum.'件', 'result' => '']);
        }
        $flashSalePurchase = $this->flashGoods['goods_num'] - $this->flashGoods['buy_num'];//抢购剩余库存
        ...
        $buyGoods['member_goods_price'] = $this->flashGoods['price'];
