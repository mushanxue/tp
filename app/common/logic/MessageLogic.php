...
class MessageLogic 
{
    ...
    public function getUserMessageCount(){
        ...
        $user_system_message_no_read = DB::name('user_message')
            ->alias('um')
            ->join('message m','um.message_id = m.message_id','LEFT')
    ...
    {
        $row = DB::name('order')->field('order_sn, shipping_name')->where('order_id', $order_id)->find();
        if (!$row) {
    ...
    public function sendMessage($msg, $push_data, $user_ids = [])
    {
        ...
        $push_ids = [];
        if (!$msg['type']) {
            $push_ids = DB::name('users')->where(['user_id' => ['IN', $user_ids]])->column('push_id');
        }
        $push = new PushLogic;
        ...
        if ($msg['type'] == 1) {
            ...
            $msg_id = DB::name('Message')->insertGetId($message);
            foreach ($user_ids as $uid) {
                DB::name('user_message')->insert(['user_id' => $uid, 'message_id' => $msg_id, 'status' => 0, 'category' => $msg['category']]);
            }
        }
        return $res;
    }
    ...
    public function setMessageSwitch($type, $val, $user)
    {
        ...
        DB::name('users')->where('user_id', $user['user_id']) ->save(['message_mask' => $user['message_mask']]);
    ...
    {
        DB::name('user_message')->where('user_id', $user_id)->save(['status' => 1]);
    }
}
