...
class MessageBase 
{
    ...
    {
        $data = $this->message;
        $arr = Db::name('user_msg_tpl')->where('mmt_code', $data['mmt_code'])->find();
    ...
    protected function sendUserMessage()
    {
        ...
        if (!empty($data['users']) && !empty($data['message_id'])) {
            foreach ($data['users'] as $key) {
                Db::name('user_message')->insert(array('user_id' => $key, 'message_id' => $data['message_id'], 'category' => $data['category']));
            }
        }
    }
