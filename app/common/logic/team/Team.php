...
class Team
{
    ...
    {
        $this->teamGoodsItem = TeamGoodsItem::where(['goods_id' => $goods_id, 'item_id' => $item_id, 'deleted' => 0])->find();
    ...
    public function setTeamActivityById($team_id)
    {
        if($team_id > 0){
            $this->teamId = $team_id;
            $this->teamActivity = TeamActivity::find($team_id);
        }
    }
    ...
    public function getTeamFollowNum(){
        $team = TeamFound::find($this->foundId);
        $need = $team['need'] - 1;
        // status=2 3 不计算
    ...
    public function setTeamFoundById($found_id)
    {
        if($found_id){
            ...
            $this->teamFound = TeamFound::find($this->foundId);
        }
    }
    ...
    public function doOrderPayAfter(){
        $teamFound = TeamFound::where(['order_id' => $this->order['order_id']])->find();
        //团长的单
        if ($teamFound) {
            ...
            //团员的单
            $teamFollow = TeamFollow::where(['order_id' => $this->order['order_id']])->find();
            if($teamFollow){
                $teamFollow->status = 1;
                $teamFollow->save();
                //更新团长的单
    ...
    public function getCouponOrderList($userCouponList)
    {
        ...
        $userCouponArray = \think\Collection::make($userCouponList)->toArray();
        $couponNewList = [];
    ...
    {
        $userCouponArray = $userCouponList;
        $couponNewList = [];
        foreach ($userCouponArray as $couponKey => $couponItem) {
            if ($this->order['goods_price'] >= $userCouponArray[$couponKey]['coupon']['condition']) {
