...
class TeamOrder
{
    ...
    public function __construct($user_id, $order_id)
    {
        ...
        $this->order = Order::where(['order_id' => $order_id, 'prom_type' => 6, 'user_id' => $user_id])->find();
        if (empty($this->order)) {
        ...
        if($this->orderGoods['spec_key']){
            $this->spec_goods_price = SpecGoodsPrice::where(['goods_id'=>$this->teamActivity['goods_id'], 'key'=>$this->orderGoods['spec_key']])->find();
        }
    }
