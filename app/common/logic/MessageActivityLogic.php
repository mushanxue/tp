...
class MessageActivityLogic extends MessageBase
{
    ...
    public function addMessage(){
        ...
        $message_id = Db::name('message_activity')->insertGetId($this->message);
    ...
    {
        $message_id = Db::name('message_activity')->where(['prom_id' => $prom_id, 'prom_type' => $type])->value('message_id');
        if ($message_id) {
            Db::name('message_activity')->where(['prom_id' => $prom_id, 'prom_type' => $type])->delete();
            Db::name('user_message')->where(['message_id' => $message_id, 'category' => 1])->delete();
        }
