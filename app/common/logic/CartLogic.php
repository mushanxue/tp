...
class CartLogic
{
    ...
    { 
        if ($goods_id > 0) {
            ...
            //加了判断，上架才能买            
            $this->goods = $goodsModel::where(['goods_id'=>$goods_id,'is_on_sale'=>1])->find();
        }
    }
    ...
    public function setSpecGoodsPriceById($item_id)
    {
        ...
        if ($item_id > 0) {
            ...
            $this->specGoodsPrice = $specGoodsPriceModel::where('item_id',$item_id)->find();
        }else{
    ...
    public function setSpecGoodsPriceByKey($key)
    {
        if ($key) {
            $specGoodsPriceModel = new SpecGoodsPrice();
            $this->specGoodsPrice = $specGoodsPriceModel::where(['goods_id' => $this->goods['goods_id'], 'key' => $key])->find();
        }else{
            $this->specGoodsPrice = null;
        }
    }
    ...
    public function buyNow()
    {
        ...
        if($this->goods['is_virtual'] == 1){
            ...
            $isBuySum = Db::name('order_goods')->alias('og')->join('order o','og.order_id = o.order_id','LEFT')->where($isBuyWhere)->sum('og.goods_num');
        ...
        $portrait->edit_cart($this->user_id,$this->session_id);
    ...
    public function addGoodsToCart()
    {
        ...
        if ($this->goods['exchange_integral'] > 0) {
            throw new TpshopException("加入购物车", 0, ['status' => 0, 'msg' => '积分商品跳转', 'result' => ['url' => url('Goods/goodsInfo', ['id' => $this->goods['goods_id'], 'item_id' => $this->specGoodsPrice['item_id']], '', true)]]);
        }
        ...
        if (empty($this->specGoodsPrice) && !empty($specGoodsPriceCount)) {
            throw new TpshopException("加入购物车", 0, ['status' => 0, 'msg' => '必须传递商品规格', 'result' => ['url' => url('Goods/goodsInfo', ['id' => $this->goods['goods_id']], '', true)]]);
        }
        //有商品规格，和没有商品规格
        ...
        $portrait = new \app\common\logic\Portrait();
        $portrait->add_cart($this->user_id,$this->goods['goods_id'],$this->goodsBuyNum,$this->session_id);
        ...
        foreach ($cartList as $cartKey => $cart) {
            ...
            if ($goodsPromFactory->checkPromType($cart['prom_type'])) {
                if (!empty($cart['spec_key'])) {
                    $specGoodsPrice = SpecGoodsPrice::where(['goods_id' => $cart['goods_id'], 'item_id' => $cart['item_id']])->find();
                ...
                if($cart['combination']['end_time'] < time() || $cart['combination']['is_on_sale']==0){
                    //删除自己的过期套餐
                    Db::name('cart')->where(['user_id'=>$cart->user_id,'prom_id'=>$cart['combination']->combination_id])->delete();
                    unset($cartList[$cartKey]);
              }
