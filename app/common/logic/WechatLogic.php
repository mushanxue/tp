...
class WechatLogic
{
            ...
            if (!empty($msg['EventKey'])) {
                ...
                if ($userData['first_leader']) {
                    ...
                    if ($first_leader) {
                        ...
                        //他上线分销的下线人数要加1
                        Db::name('users')->where('user_id', $userData['first_leader'])->inc('underling_number')->update();
                        Db::name('users')->where('user_id', $userData['second_leader'])->inc('underling_number')->update();
                        Db::name('users')->where('user_id', $userData['third_leader'])->inc('underling_number')->update();
                    }
                } else {
    ...
    private function createReplyMsg($from, $to, $type, $data = [])
    {
        if ($type != WxReply::TYPE_KEYWORD) {
            $reply = WxReply::where(['type' => $type])->find();
        } else {
            $wx_keyword = WxKeyword::where(['keyword' => $data['keyword'], 'type' => WxKeyword::TYPE_AUTO_REPLY], 'wxReply')->find();
    ...
    private function replyMyQrcode($msg)
    {
        ...
        $user = Db::name('oauth_users')->alias('o')->join('users u', 'u.user_id=o.user_id')
    ...
    private function replyKeyword($from, $to, $keyword)
    {
        ...
        if($keyword == '客服'){
            $return = self::$wechat_obj->duokefu($from, $to);
            exit($return);
        }
    ...
    {
        $material = WxMaterial::where(['id' => $material_id, 'type' => WxMaterial::TYPE_NEWS], 'wxNews')->find();
    ...
    private function createShareQrCode($backImg, $qrText, $headPic)
    {
        ...
        if ($backImg && is_file($backImg)) {
            $back = \think\image\Image::open($backImg);
        ...
        if ($headPic) {
            ...
            if (file_exists($headPic)) {
                $logo = \think\image\Image::open($headPic);
    ...
    public function getFanList($p, $num = 10)
    {
        ...
        foreach ($part_fans as $openid) {
            ...
            //查user_id
            $user_id = Db::name('oauth_users')->where(['openid'=>$openid])->value('user_id');
    ...
    public function getUserFanList($p, $num = 10, $keyword= '')
    {
        ...
        $query = Db::name('oauth_users')->field('o.*')->alias('o')->join('users u', 'u.user_id = o.user_id')->where($condition);
    ...
    public function createOrUpdateText($material_id, $data)
    {
        ...
        if ($material_id) {
            if (!$material = WxMaterial::where(['id' => $material_id, 'type' => WxMaterial::TYPE_TEXT])->find()) {
    ...
    {
        if (!$material_id || !$material = WxMaterial::where(['id' => $material_id, 'type' => WxMaterial::TYPE_TEXT])->find()) {
            return ['status' => -1, 'msg' => '文本素材不存在'];
        }
    ...
    public function createOrUpdateNews($material_id, $news_id, $data)
    {
        ...
        if ($material_id) {
            if (!$material = WxMaterial::where(['id' => $material_id, 'type' => WxMaterial::TYPE_NEWS])->find()) {
                return ['status' => -1, 'msg' => '图文素材不存在'];
            }

            if ($news_id) {
                //更新单图文
                if (!$news = WxNews::where(['id' => $news_id, 'material_id' => $material_id])->find()) {
                ...
                //新增单图文
                $all_news = WxNews::where(['material_id' => $material_id])->select();
    ...
    {
        if (!$material_id || !$material = WxMaterial::where(['id' => $material_id, 'type' => WxMaterial::TYPE_NEWS], 'wxNews')->find()) {
            return ['status' => -1, 'msg' => '素材不存在'];
        }

        if (WxReply::where(['material_id' => $material_id, 'msg_type' => WxReply::MSG_NEWS])->find()) {
    ...
    {
        if (!$news_id || !$news = WxNews::where($news_id, 'wxMaterial')->find()) {
            return ['status' => -1, 'msg' => '单图文素材不存在'];
        }
    ...
    private function uploadNews($material)
    {
        ...
        foreach ($material->wx_news as $news) {
            // 1.获取或上传封面
            if ($thumb = WxMaterial::where(['type' => WxMaterial::TYPE_IMAGE, 'key' => md5($news['thumb_url'])])->find()) {
            ...
            if (preg_match_all('#<img .*?src="(.*?)".*?/>#i', $news['content'], $matches)) {
                ...
                foreach ($imgs as $img) {
                    ...
                    // 3.获取或上传文章中图片
                    if ($news_image = WxMaterial::where(['type' => WxMaterial::TYPE_NEWS_IMAGE, 'key' => md5($img)])->find()) {
    ...
    public function deleteImage($url)
    {
        ...
        if (!$image = WxMaterial::where(['type' => WxMaterial::TYPE_IMAGE, 'key' => md5($url)])->find()) {
            return;
        }
        if ($image->media_id) {
    ...
    public function setTemplateMsg($template_sn, $data)
    {
        ...
        if ($tpl_msg = WxTplMsg::where(['template_sn' => $template_sn])->find()) {
            $tpl_msg->save($data);
        } else {
    ...
    public function sendTemplateMsgOnTeam($found_id){
        ...
        $tpl_msg = WxTplMsg::where(['template_sn' => $template_sn, 'is_use' => 1])->find();
    ...
    public function sendTemplateMsgOnPaySuccess($order)
    {
		...
		if($oldOrder['prom_type'] == 4){
            // 部分订金支付
            if($oldOrder['pay_status'] != 1 ){
                $order['total_amount'] = $oldOrder['paid_money'];
            }else{
                $order['total_amount'] = $oldOrder['total_amount'] - $oldOrder['paid_money'];
            }
        }
        $data = [
            'first' => ['value' => '订单支付成功！'],
            ...
            'keyword5' => ['value' => $order['total_amount']],  //订单总金额，而不是订单应付款金额
            'remark' => ['value' => $tpl_msg->remark ?: ''],
        ];
    ...
    public function addTextAutoReply($data)
    {
        ...
        if ($data['type'] == WxReply::TYPE_KEYWORD) {
            if (WxKeyword::where([['keyword' , 'in', $data['keywords']], 'type' => WxKeyword::TYPE_AUTO_REPLY])->find()) {
    ...
    public function updateTextAutoReply($reply_id, $data)
    {
        ...
        if (!$reply = WxReply::where(['id' => $reply_id], $with)->find()) {
            return ['status' => -1, 'msg' => '该自动回复不存在'];
        }

        if ($data['type'] == WxReply::TYPE_KEYWORD) {
            $keyword_ids = get_arr_column($reply->wx_keywords, 'id');
            if (WxKeyword::where([['keyword' , 'in', $data['keywords']], 'type' => WxKeyword::TYPE_AUTO_REPLY, 'id' => ['not in', $keyword_ids]])->find()) {
                return ['status' => -1, 'msg' => '有关键字被其他规则使用22222'];
            }
    ...
    private function checkNewsAutoReplyForm(&$data)
    {
        ...
        if (!WxMaterial::where(['id' => $data['material_id'], 'type' => WxMaterial::TYPE_NEWS])->find()) {
    ...
    public function updateNewsAutoReply($reply_id, $data)
    {
        ...
        if ($data['type'] == WxReply::TYPE_KEYWORD) {
            $keyword_ids = get_arr_column($reply->wx_keywords, 'id');
            if (WxKeyword::where([['keyword' , 'in', $data['keywords']], 'type' => WxKeyword::TYPE_AUTO_REPLY, 'id' => ['not in', $keyword_ids]])->select()) {
                return ['status' => -1, 'msg' => '有关键字被其他规则使用'];
            }
    ...
    {
        if (!$reply = WxReply::where(['id' => $reply_id])->find()) {
            return ['status' => -1, 'msg' => '该自动回复不存在'];
        }
    ...
    public function deleteAutoReplys($reliesIds)
    {
        ...
        return ['status' => 1,'msg' => '操作成功','url'=>url("Admin/Wechat/auto_reply")];
    }
}