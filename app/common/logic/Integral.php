...
class Integral
{
    ...
    public function setSpecGoodsPriceById($item_id){
        if($item_id > 0){
            $this->specGoodsPrice = SpecGoodsPrice::find($item_id);
        }
    }
    ...
    public function setUserAddressById($address_id){
        if($address_id > 0){
            $this->userAddress = UserAddress::find($address_id);
        }
        return $this;
    }
    ...
    public function checkBuy()
    {
        ...
        if (empty($this->specGoodsPrice)) {
            $goods_spec_list = SpecGoodsPrice::where(['goods_id' => $this->goods['goods_id']])->select();
            if (!$goods_spec_list->isEmpty()) {
