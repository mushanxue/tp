...
class GoodsLogic 
{
    ...
    public function get_filter_brand($goods_id_arr, $filter_param, $action)
    {
        ...
        $list_brand = Db::name('brand')->where([['id','in',$brand_id_arr]])->limit(30)->select()->toArray();
    ...
    public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        ...
        $spec_item = Db::name('spec_item')->where(array('spec_id'=>array('in',array_keys($spec))))->column('id,spec_id,item','id');
    ...
    public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0)
    {
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $goods_attr = Db::name('goods_attr')->where([['goods_id','in',$goods_id_str],['attr_value','<>','']])->select()->toArray();        
        $goods_attribute = Db::name('goods_attribute')->where("attr_index = 1")->column('attr_id,attr_name,attr_index','attr_id');
        if (empty($goods_attr)) {
            if ($mode == 1) return array();
            return array('status' => 1, 'msg' => '', 'result' => array());
        }
    ...
    public function collect_goods($user_id, $goods_id)
    {
        ...
        if($collect_id){
            $is_collect = 0;
            $portrait->add_collect($user_id, $goods_id,$is_collect);
            Db::name('goods')->where('goods_id', $goods_id)->dec('collect_sum')->update();
            Db::name('goods_collect')->where('collect_id',$collect_id)->delete();
            return array('status'=>2,'msg'=>'已取消收藏!','result'=>array());
        }else{
            $is_collect = 1;
            $portrait->add_collect($user_id, $goods_id,$is_collect);
            Db::name('goods')->where('goods_id', $goods_id)->inc('collect_sum')->update();
    ...
    public function get_spec($goods_id,$terminal="pc")
    {
        ...
        $filter_spec = array();
        if ($keys) {
            $specImage = Db::name('SpecImage')->where(['goods_id'=>$goods_id,['src','<>','']])->column('src',"spec_image_id");// 规格对应的 图片表， 例如颜色
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if ($i == 0)
                $parr[] = array('value' => "{$end}元以下", 'href' => urldecode(url("Goods/$action", $filter_param, '')),'param'=>$filter_param['price']);
            elseif($i == ($c-1) && ($max_price > $end))
                $parr[] = array('value' => "{$end}元以上", 'href' => urldecode(url("Goods/$action", $filter_param, '')),'param'=>$filter_param['price']);
            else
                $parr[] = array('value' => "{$start}-{$end}元", 'href' => urldecode(url("Goods/$action", $filter_param, '')),'param'=>$filter_param['price']);
        }
        return $parr;
    }
    ...
    {
        $res = Cache('getRegionList');
        if(!empty($res))
            return $res;
        $parent_region = Db::name('region')->field('id,name')->where(array('level'=>1))->cache(true)->select()->toArray();
        $ip_location = array();
        $city_location = array();
    ...
    public function getGoodsPriceByLadder($goods_num, $goods_price, $price_ladder)
    {
        ...
        for ($i = 0; $i < $price_ladder_count; $i++) {
            ...
            if($goods_num >= $price_ladder[$i]['amount'] && $goods_num < $price_ladder[$i+1]['amount']){
                return round($price_ladder[$i]['price'],2);
            }
    ...
    public function getPromotionGoods()
    {
        ...
        $promotion_goods = Db::name('goods')
            ...
            ->join('flash_sale f','g.goods_id = f.goods_id')
            ->where($goods_where)
            ->limit(3)
            ->select();
    ...
    public function getGoodsFavorableRate($goods_id){
        ...
        $ok = Db::name('comment')->where('goods_id',$goods_id)->where('goods_rank','>',3)->count();
...
//        $goods_where['goods_name'] = array("exp", " NOT REGEXP '华为|荣耀|小米|合约机|三星|魅族' ");//临时屏蔽,苹果APP审核过了之后注释
        $favourite_goods = Db::name('goods')
            ...
            ->alias('a')
            ->join('goods_label b','a.label_id = b.label_id','left')
            ->where($goods_where)
            ->order('a.sort DESC')
            ->page($p, 10)
            ->cache(true, 3600)
    ...
    {
        $where['f.start_time'] = array('>=', $start_time ?: time());
        if ($end_time) {
            $where['f.end_time'] = array('<=',$end_time);
        }
        $where['f.is_end'] = 0;
        $flash_sale_goods = Db::name('flash_sale')->alias('f')
            ->field('f.end_time,f.goods_num,f.buy_num,f.goods_name,f.price,f.goods_id,f.price,g.shop_price,f.item_id,100*(FORMAT(f.buy_num/f.goods_num,2)) as percent,buy_num')
    ...
    public function integralMall($rank, $user_id, $p = 1)
    {
        ...
        //积分兑换筛选
        //$exchange_integral_where_array = array(array('>',0));
    ...
    function getSortBrands()
    {
        ...
        $brandIdArr =  Db::name("Brand")->where("name in (select `name` from `".config('database.connections.mysql.prefix')."brand` group by name having COUNT(id) > 1)")->column('cat_id','id');
        $goodsCategoryArr = Db::name('goodsCategory')->where("level = 1")->column('name','id');
        $nameList = array();
    ...
    public function ajax_category($category=2){
        ...
        if($category == 2){
            ...
            $list['goods'] = Db::name('goods')
                ->field('goods_id,goods_name,cat_id,sales_sum,virtual_sales_sum,original_img,shop_price,comment_count')
     ...
     * 商品分享海报
     * @param $data
     * @param int $type | 0默认为小程序，2用于手机端海报
     */
    ...
    private function exportGoodsSharePoster($data){
        ...
        $goods_path = ROOT_PATH.'public'.goods_thum_images($data['goods_id'], 500, 500,0);
    ...
    private function exportGoodsSharePoster2($data){
        ...
        if(empty($data['head_pic'])){
            $data['head_pic'] = '/template/mobile/rainbow/static/images/distrbut/account_header@2x.png';
        }
        if(strpos($data['head_pic'],'/') === 0){
            ...
            if($str){
                ...
                $head_pic = imagecreatefromstring(file_get_contents('./template/mobile/rainbow/static/images/distrbut/account_header@2x.png'));
            }
        }
