...
class CouponLogic 
{
    ...
    public function getSendValidCouponMoney($coupon_id, $goods_id, $cat_id)
    {
        ...
        $goods_coupon = DB::name('goods_coupon')->where('coupon_id', $coupon_id)->where(function ($query) use ($goods_id, $cat_id) {
    ...
    public function getCartCouponList($user_id, $type, $cartList, $p = 1, $size = null)
    {
        ...
        $query = Db::name('coupon')->alias('c1')
                ...
                ->join('coupon_list c2','c2.cid=c1.id AND c2.status=0', 'LEFT')
                ->where($where);
        if ($size) {
    ...
    public function getUserAbleCouponList($user_id, $goods_ids = array(), $goods_cat_id = array())
    {
        ...
        $Coupon = new Coupon();
        $userCouponArr = [];
        $userCouponList = $CouponList->where('uid', $user_id)->where('status', 0)->select()->toArray();//用户优惠券
        if(!$userCouponList){
            ...
            foreach ($couponList as $coupon => $couponItem) {
                if ($userCouponItem['cid'] == $couponItem['id']) {
                    ...
                    if ($couponItem['use_type'] == 2 && !empty($couponItem['goods_coupon'])) {
                        //追溯上级分类，优惠券
                        $all_cat = Db::name('goods_category')->where([['id','in',$goods_cat_id]])->field('parent_id_path')->select();
                        $arr = [];
    ...
    public function exchangeCoupon($user_id, $coupon_code)
    {
        ...
        $coupon = Coupon::find($coupon_list['cid']); // 获取优惠券类型表
        if (time() < $coupon['use_start_time']) {
