...
class MessageNoticeLogic extends MessageBase
{
    ...
    public function addMessage(){
        ...
        $message_id = Db::name('message_notice')->insertGetId($this->message);
        if($message_id) {
    ...
    {
        $message_id = Db::name('message_notice')->where(['prom_id' => $prom_id, 'type' => $type])->value('message_id');
        if ($message_id) {
            Db::name('message_notice')->where(['prom_id' => $prom_id, 'type' => $type])->delete();
            Db::name('user_message')->where(['message_id' => $message_id, 'category' => 0])->delete();
        }
    }
    ...
    public function getCouponNotice($cid, $uid_arr)
    {
        ...
        $name = Db::name('coupon')->where('id', $cid)->value('name');
    ...
    public function useCouponNotice($order_id, $couponId, $user_id)
    {
        ...
        $data['users'] = [$user_id];
        $cid = Db::name('coupon_list')->where('id', $couponId)->value('cid');
        $data['message_content'] = Db::name('coupon')->where('id', $cid)->value('name');
        $this->sendMessageNotice($data);
    }
    ...
    public function delCouponNotice($cid)
    {
        ...
        $uid = Db::name('coupon_list')->where('cid', $cid)->value('uid');
        if ($uid) {
            $where['type'] = 2;
            $where['prom_id'] = $cid;
            $message_id = Db::name('message_notice')->where($where)->column('message_id');
            if ($message_id) {
                Db::name('message_notice')->where($where)->delete();
                Db::name('user_message')->where([['message_id' , 'in', $message_id], 'category' => 0])->delete();
            }
        }
    }
    ...
    public function couponWillExpireNotice($cid, $user_id)
    {
        ...
        $message_id_arr = Db::name('message_notice')->where($where)->column('message_id');
        $where_message['category'] = 0;
        $where_message['user_id'] = $user_id;
        $where_message['message_id'] = ['in', $message_id_arr];
        $message_id = Db::name('user_message')->where($where_message)->value('message_id');
        if ($message_id) {
            return;
        }
        ...
        $message_content = Db::name('coupon')->where('id', $cid)->value('name');
        $data['users'] = [$user_id];
