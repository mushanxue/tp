...
class Message
{
    ...
    public function getUserMessageCount(){
        ...
        // 通知消息未查看数
        $data['message_notice_no_read'] = Db::name("user_message")->where($where)->count();
        // 活动消息未查看数
        $where['category'] = 1;
        $data['message_activity_no_read'] = Db::name("user_message")->where($where)->count();
        // 物流消息未查看数
        $where['category'] = 2;
        $data['message_logistics_no_read'] = Db::name("user_message")->where($where)->count();
        // 私信未查看数
        $where['category'] = 3;
        $data['message_private_no_read'] = Db::name("user_message")->where($where)->count();
        return $data;
    }
    /*
     * 获得用户所有未查看的消息数
    ...
    public function getNoRead(){
        ...
        // 通知消息未查看数
        $count = Db::name("user_message")->where($where)->count();
        return $count;
    }
    /**
     * 未查看的消息总数 pc和手机用
    ...
    /**
     * 按发送时间排序
     * @param $rec_id | $rec_id = Db::name("user_message")->where($where)->column('rec_id');
     * @param $type | 消息类型
     * @return array
     */
    ...
    public function checkUserMessage($user_id)
    {
        ...
        if ($user_info) {
            ...
            $message_where = array(
                'message_type' => 1,
                'send_time' => array('>', $user_info['reg_time']),
            );
    ...
    public function setMessageForRead($rec_id)
    {
        if (!empty($this->userId)) {
            ...
            $result = Db::name('user_message')->where($set_where)->update($data);
            if ($result) {
