...
class ActivityLogic
{
    ...
    public function getGroupBuyList($sort_type = '', $page_index = 1, $page_size = 20)
    {
        ...
        $list = Db::name('group_buy')->alias('b')
    ...
    public function getSalesList($sort_type = 0, $page_index = 1, $page_size = 4)
    {
        ...
        $list = Db::name('prom_goods')->with(['prom_goods_item'])->where($map)->field('id,type,expression,end_time')->page($page_index , $page_size)->order('id desc')->select();
        $list && $list->append(['prom_detail'])->toArray();
        return $list;
    }
    ...
    {
        $time = time();
        $where = array('type' => 2,'status'=>1,'send_start_time'=>['<=',time()],'send_end_time'=>['>=',time()]);
        $order = array('id' => 'desc');
        if ($atype == 2) {
            //即将过期
            $order = ['spacing_time' => 'asc'];
            $where[] = [Db::raw("send_end_time-'$time'"), '>=', 0];
        } elseif ($atype == 3) {
            //面值最大
            $order = ['money' => 'desc'];
        }
        ...
        if (!$coupon_list->isEmpty()) {
            $coupon_list = $coupon_list->append(['coupon_img','use_type_title'])->toArray();
            if ($user_id) {
                $user_coupon = Db::name('coupon_list')->where(['uid' => $user_id, 'type' => 2])->column('cid');
            }
            if (!empty($user_coupon)) {
                foreach ($coupon_list as $k => $val) {
                    ...
                    $coupon_list[$k]['use_scope'] = config('COUPON_USER_TYPE')[$coupon_list[$k]['use_type']];
                }
            }
        }
        return $coupon_list;
    ...
    public function getCouponQuery($queryType, $user_id, $type = 0, $orderBy = null , $order_money = 0)
    {
        ...
        if (empty($type)) {
            // 未使用
            $where['l.order_id'] = 0;
            $where['c.use_end_time'] = array('>', time());
            $where['l.status'] = 0;
        } elseif ($type == 1) {
            //已使用
            $where['l.order_id'] = array('>', 0);
            $where['l.use_time'] = array('>', 0);
            $where['l.status'] = 1;
        } elseif ($type == 2) {
            //已过期
            $where['c.use_end_time'] = array('<', time());
            $where['l.status|c.status'] = array('<>', 1);
        }
        ...
        $query = Db::name('coupon_list')->alias('l')
            ->join('coupon c','l.cid = c.id'.$condition)
            ->where($where);
    ...
    public function getUserCouponList($firstRow, $listRows, $user_id, $type = 0, $orderBy = null,$order_money = 0)
    {
        ...
        return $query->limit($firstRow, $listRows)->select()->toArray();
    }
    ...
    public function getCouponCenterList($cat_id, $user_id, $p = 1,$goods_id=0)
    {
        /* 获取优惠券列表 */
        $cur_time = time();
        $coupon_where = ['type'=>2, 'status'=>1, 'send_start_time'=>['<=',time()], 'send_end_time'=>['>=',time()]];
...
//        if ($cat_id > 0) {
//            $query = $query->join('goods_coupon gc', 'gc.coupon_id=c.id AND gc.goods_category_id='.$cat_id);
//        }
        $query = $query->join('goods_coupon gc', 'gc.coupon_id=c.id ','left');
        ...
        if(!empty($coupon_list)) {
            ...
            foreach($coupon_list as $key =>$value){
                if ($value["goods_category_id"]) {
                    $cate_name = Db::name('GoodsCategory')->cache(true, TPSHOP_CACHE_TIME)->where(["id" => $value["goods_category_id"]])->value('name');
                    if ($cate_name) {
                        $coupon_list[$key]['goods_category_name'] = $cate_name;
                    }
                }
                else{
                    $coupon_list[$key]['goods_category_name'] = null;
                }
            }
        }
        return $coupon_list;
    ...
    public function getCouponTypes($p = 1, $num = null)
    {
        $list = Db::name('coupon')->alias('c')
                ->join('goods_coupon gc', 'gc.coupon_id=c.id AND gc.goods_category_id!=0')
                ->where(['type' => 2, 'status' => 1])
                ->column('gc.goods_category_id');
        
        $result = Db::name('goods_category')->field('id, mobile_name')->where("id", "IN", $list)->page($p, $num)->select()->toArray();
        $result = $result ?: [];
    ...
    public function getGoodsPromSimpleInfo($user, &$goods)
    {
        ...
        $goods  = Db::name('Goods')->where('goods_id', $goods['goods_id'])->find();
