...
class PreSellLogic extends Prom
{
    ...
    public function initProm()
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，规格和活动是一对一
            $this->preSell = PreSell::find($this->specGoodsPrice['prom_id']);
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->preSell = PreSell::find($this->goods['prom_id']);
        }
        if ($this->preSell) {
            ...
            if ($this->checkActivityIsEnd() && $this->preSell['is_finished'] == 0) {
                if($this->specGoodsPrice){
                    ...
                    $this->specGoodsPrice = SpecGoodsPrice::find($item_id['item_id']);
                }else{
