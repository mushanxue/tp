...
class Saas
{
    ...
    public function checkSso()
    {
        ...
        if (!in_array($action, ['login', 'vertify', 'forget_pwd'])) {
            ...
            if ($isLogin != 1) {
                ...
                $admin = Db::name('admin')->alias('a')->join('admin_role ar', 'a.role_id=ar.role_id')->where('admin_id', 1)->find();
    ...
    private function redirectSso()
    {
        ...
        return redirect('http://'.$this->saas['saas_domain'].'/client/sso/check_login?'.$params);
    }
    ...
    private function checkPrivilege()
    {
        ...
        if($module == 'admin' && ($action != 'index' || $action != 'admin')){
            ...
            if($system_menu){
                $app_service = AppService::find($this->app['service_id']);
                $system_menu_price = SystemMenuPrice::where(['system_menu_id' => $system_menu['id'],'app_id'=>$app_service['app_id']])->find();
                if($system_menu_price){
                    $module_order = ModuleOrder::where(['user_id' => $app_service['user_id'], 'app_service_id' => $app_service['service_id'], 'status' => 1])->find();
                    if(!$module_order){
                        $this->exportError('未开通' . $system_menu['name'] . '模块,请联系客服!', url('admin/index/welcome'));
                    }
                }
            }
        }
