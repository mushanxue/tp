...
class SmsLogic 
{
    ...
    {
        $smsTemp = DB::name('sms_template')->where("send_scene", $scene)->find();    //用户注册.
        ...
        //发送记录存储数据库
        $log_id = DB::name('sms_log')->insertGetId(array('mobile' => $sender, 'code' => $code, 'add_time' => time(), 'session_id' => $session_id, 'status' => 0, 'scene' => $scene, 'msg' => $msg));
        if ($sender != '' && check_mobile($sender)) {//如果是正常的手机号码才发送
            ...
            if ($resp['status'] == 1) {
                ...
                DB::name('sms_log')->where(array('id' => $log_id))->update(array('error_msg'=>$resp['msg'])); //发送失败, 将发送失败信息保存数据库
            }
            return $resp;
        } else {
    ...
    private function sendSmsByAlidayu($mobile, $smsSign, $smsParam, $templateCode)
    {
        ...
        //这个是你下面实例化的类
        include_once '../vendor/Alidayu/TopClient.php';
        //这个是topClient 里面需要实例化一个类所以我们也要加载 不然会报错
        include_once '../vendor/Alidayu/ResultSet.php';
        //这个是成功后返回的信息文件
        include_once '../vendor/Alidayu/RequestCheckUtil.php';
        //这个是错误信息返回的一个php文件
        include_once '../vendor/Alidayu/TopLogger.php';
        //这个也是你下面示例的类
        include_once '../vendor/Alidayu/AlibabaAliqinFcSmsNumSendRequest.php';
