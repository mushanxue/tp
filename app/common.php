...
function goods_thum_images($goods_id, $width, $height,$item_id=0)
{
    ...
    try {
        ...
        $image = \think\image\Image::open($original_img);
...
function minus_stock($order){
    $orderGoodsArr = Db::name('OrderGoods')->master(true)->where("order_id", $order['order_id'])->select();
    foreach($orderGoodsArr as $key => $val)
    {
        ...
        {   // 先到规格表里面扣除数量 再重新刷新一个 这件商品的总数量
            ...
            if($val['prom_type'] == 6){
                Db::name('team_goods_item')->where(['item_id' => $specGoodsPrice['item_id'], 'deleted' => 0])->inc('sales_sum', $val['goods_num'])->update();
            }
        }else{
            $specGoodsPrice = null;
            Db::name('Goods')->where("goods_id", $val['goods_id'])->dec('store_count',$val['goods_num'])->update(); // 直接扣除商品总数量
        }
        Db::name('Goods')->where("goods_id", $val['goods_id'])->inc('sales_sum',$val['goods_num'])->update(); // 增加商品销售量
        //更新活动商品购买量
        ...
        //更新拼团商品购买量
        if($val['prom_type'] == 6){
            Db::name('team_activity')->where('team_id',  $val['prom_id'])->inc('sales_sum', $val['goods_num'])->update();
        }elseif($val['prom_type'] == 8){
            // 增加砍价购买量
            Db::name('promotion_bargain_goods_item')->where('bargain_id',  $val['prom_id'])->inc('buy_num', $val['goods_num'])->update();
        }
...
function send_email($to,$subject='',$content=''){
    include_once '../vendor/phpmailer/PHPMailerAutoload.php'; ////require_once vendor/phpmailer/PHPMailerAutoload.php';
    //判断openssl是否开启
...
function getCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['category_id_arr'] = Db::name('GoodsCategory')->cache(true,TPSHOP_CACHE_TIME)->column('parent_id','id');
    // 先把所有儿子找出来
    $son_id_arr = Db::name('GoodsCategory')->where("parent_id", $cat_id)->cache(true,TPSHOP_CACHE_TIME)->column('id');
...
function getArticleCatGrandson ($cat_id)
{
    ...
    // 把整张表找出来
    $GLOBALS['cat_id_arr'] = Db::name('ArticleCat')->column('parent_id','cat_id');
    // 先把所有儿子找出来
...
//    }
    ...
    if($recharge) $update_data['user_total_money'] = Db::raw('user_total_money+'.$user_money);  //用户充值累计金额
    if($withdrawn) $update_data['withdrawal_total_money'] = Db::raw('withdrawal_total_money+'.$withdrawn);  //用户提现累计金额
...
function accountDistributLog($user_id,$pay_points = 0, $desc = '',$withdrawals_money = 0,$order_id = 0 ,$order_sn = '',$distribut_money = 0){
    ...
    /* 更新用户信息 */
    $update_data = array(
        'distribut_withdrawals_money'   => Db::raw('distribut_withdrawals_money+'.$withdrawals_money), //提现
        'distribut_money'   => Db::raw('distribut_money+'.$distribut_money),     //获得佣金
    );

    if($distribut_money+$withdrawals_money == 0) return false;
    $update = Db::name('users')->where("user_id = $user_id")->save($update_data);
    if($update){
        Db::name('account_distribut_log')->insert($account_log);
        return true;
    }else{
        return false;
    }
...
function update_pay_status($order_sn,$ext=array())
{
	...
	$recharge=Db::name('recharge')->where(['order_sn'=>$order_sn])->find();
        if($order['level_id']>0){
            Db::name('users')->where('user_id',$order['user_id'])->save(array('distribut_level'=>$order['level_id']));
            $level = Db::name('distribut_level')->where('level_id',$order['level_id'])->find();
            if($level['reward']>0){
...
//            if($card['give']==0){
//                Db::name('shopping_card_list')->where(['id'=>$order['card_list_id']])->inc('balance',($distribut['targer_money']+$distribut['give_num']));
//            }else if($card['give']==1){
//                Db::name('shopping_card_list')->where(['id'=>$order['card_list_id']])->inc('balance',$distribut['targer_money'])->update();
//            }
        ...
        //用户开通会员卡
        $order = Db::name('card_buy')->where(['order_sn' => $order_sn, 'pay_status' => 0])->find();
        if (!$order) return false;// 看看有没已经处理过这笔订单  支付宝返回不重复处理操作
        Db::name('card_buy')->where("order_sn",$order_sn)->save(array('pay_status'=>1,'pay_time'=>$time));
        if($check = Db::name('user_card')->where(['card_id' => $order['card_id'] , 'user_id' => $order['user_id'] ])->find()){
           //重新续费
        ...
        // 如果这笔订单已经处理过了
        $count = Db::name('order')->master(true)->where("order_sn = :order_sn and (pay_status = 0 OR pay_status = 2)")->bind(['order_sn'=>$order_sn])->count();   // 看看有没已经处理过这笔订单  支付宝返回不重复处理操作
        if($count == 0) return false;
        // 找出对应的订单
        $Order = new \app\common\model\Order();
        $order = $Order->master(true)->where("order_sn",$order_sn)->find();
        // 支付的转化率设置下
        $portrait = new \app\common\logic\Portrait();
        $portrait->collect_pay($order['user_id'],$order['order_id']);
        ...
        if ($order['prom_type'] == 4) {
            ...
            Db::name('order')->where("order_sn", $order_sn)->save($update);
        }
        ...
        if(tpCache('shopping.reduce') == 2) {
            if ($order['prom_type'] == 6) {
                $team = \app\common\model\TeamActivity::find($order['prom_id']);
                if ($team['team_type'] != 2) {
                    minus_stock($order);
                }
            } else {
		...
		if (count($newOrder) > 0) {
			...
			Db::name('rebate_log')->where("order_id", 'in', $newOrderIds)->save(array('status'=>1));
			Db::name('rebate_log')->where("order_id",$order['order_id'])->delete();
		} else {
        ...
        //如果配置了接收微信openid则发送通知 by wangqh.2019-9-24 @{
        $wxUser = Db::name('wx_user')->where('1 = 1')->find();
        if($wxUser && $wxUser['bell_openid']){
            $openids = explode("|", $wxUser['bell_openid']); 
            $wechatObj = new \app\common\logic\wechat\WechatUtil($wxUser);
            $msg = "您有新订单(订单编号:{$order['order_sn']})待发货. 收货人：{$order['consignee']},联系方式：{$order['mobile']}，请及时处理发货!";
            //使用 $wechatObj->sendMsg($openids, 'text', $msg);方法无法收到消息, 很奇怪的问题 
            foreach($openids as $openid){
                $wechatObj->sendMsg($openid, 'text', $msg);
            }
        }
        //@}
    }
...
function confirm_order($id,$user_id = 0){
    ...
    // 商品待评价提醒
    $order_goods = Db::name('order_goods')->field('goods_id,goods_name,rec_id')->where(["order_id" => $id])->find();
    ...
    //分销设置
    Db::name('rebate_log')->where(['order_id' => $id, 'status' => 1 ])->save(array('status' => 2, 'confirm' => time()));
    return array('status'=>1,'msg'=>'操作成功','url'=>url('Order/order_detail',['id'=>$id]));
}
...
function order_give($order)
{
        ...
        if ($prom_goods) {
            ...
            if ($goods_coupon) {
                ...
                if ($goods_coupon['createnum'] == 0 || ($goods_coupon['createnum']>0 && ($goods_coupon['createnum']-$goods_coupon['send_num'])>0)){
                    ...
                    // 优惠券领取数量加一
                    Db::name('Coupon')->where("id", $goods_coupon['id'])->inc('send_num')->update();
            ...
            if ($prom_order['type'] == 3) {
                ...
                if ($order_coupon) {
                    ...
                    if ($order_coupon['createnum'] == 0 ||
                        ...
                        Db::name('coupon_list')->insert($data);
                        Db::name('Coupon')->where("id", $order_coupon['id'])->inc('send_num')->update(); // 优惠券领取数量加一
                        // 优惠券到账提醒
    ...
    $points = Db::name('order_goods')->where("order_id", $order['order_id'])->sum(Db::raw("give_integral * goods_num"));
...
function write_html_cache($html){
    ...
    foreach($html_cache_arr as $key=>$val)
    {
        ...
        \think\facade\Cache::set($filename,$html);
...
function read_html_cache(){
    ...
    foreach($html_cache_arr as $key=>$val)
    {
        ...
        if($html)
        {
            ...
            echo \think\facade\Cache::get($filename).cache_str($html);
            exit();
        }
    }
}
...
{      
    ...
    if($object_ess)
    {
            if(config('buy_version') == 0)
            return '';
            $tabName = '';
            $table_index = Db::name('config')->cache(true)->select();            
      ...
      if(empty($isset_requestjs))
      {
          ...
          if($axure_rest)
          {
                    $regions = null;
                    if (!$regions) {
                        $regions = Db::name('region')->cache(true)->column('name','id');
                    }
 ...
 */
function clearCache(){
    $team_found_queue = \think\facade\Cache::get('team_found_queue');
    $urgent_pay = \think\facade\Cache::get('urgent_pay');
    //delFile(RUNTIME_PATH);
    \think\facade\Cache::clear();
    \think\facade\Cache::set('team_found_queue', $team_found_queue);
    \think\facade\Cache::set('urgent_pay', $urgent_pay);
}
...
{
    if(config('buy_version') == 1)
        return input('select_year');
    else
        return '';
}
...
function getTabByOrdersn($order_sn)
{
    if(config('buy_version') == 0)
        return '';
    $tabName = '';
    $table_index = Db::name('table_index')->cache(true)->select();
    // 截取年月日时分秒
    $select_year = substr($order_sn, 0, 14);
    foreach($table_index as $k => $v)
    {
...
function getTabByTime($startTime='', $endTime='')
{
    ...
    {
        $table_index = Db::name('table_index')->where("name = 'order'")->cache(true)->find();
...
function createImagesName(){
    return md5(input('_saas_app','all').time().rand(1000, 9999) . uniqid());
}
...
function get_bd_url($data,$type){
    switch ($type){
        case 3:
            $url = url('mobile/goods/goodsInfo',array('id'=>$data));
            break;
        case 4:
            $url = url('mobile/goods/goodsList',array('id'=>$data));
            break;
        default:
            $url = $data;
            break;
...
function user_login($user_id) 
{
    ...
    $result = Db::name('user_login')->insertGetId($data);
}
...
function supplier_account_log($suppliers_id, $supplier_money = 0, $desc = '')
{
    ...
    $update_data = array(
        'supplier_money' => Db::raw('supplier_money+' . $supplier_money)
    );
    $update = Db::name('suppliers')->where('suppliers_id', $suppliers_id)->update($update_data);
    if ($update) {
        Db::name('account_log_supplier')->insert($account_log);
        return true;
    } else {
        return false;
    }
...
function coupon_get_cate_name($cate){
     return Db::name('GoodsCategory')->cache(true,TPSHOP_CACHE_TIME)->where(["id"=>$cate])->value('name');
}
/**
 * 生成订单编号
...
//        $order_sn_count = Db::name('shopping_card_list')->where("sn = '$sn' ")->count();
...
function logOrder($order_id, $action_note, $status_desc, $user_id = 0, $user_type = 0)
{
    ...
    $order = Db::name('order')->master(true)->where("order_id", $order_id)->find();
    $action_info = array(
        'order_id' => $order_id,
        'action_user' => $user_id,
        'user_type' => $user_type,
