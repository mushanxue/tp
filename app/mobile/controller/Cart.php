...
class Cart extends MobileBase {
    ...
    public function index()
    {
        ...
        $hot_goods = Db::name('Goods')->where('is_hot=1 and is_on_sale = 1')->limit(20)->order('goods_id DESC')->cache(true, TPSHOP_CACHE_TIME)->select();
    ...
    public function cart2(){
        ...
        $is_use_integral = Db::name("config")->where("name='is_use_integral'")->value('value');//积分使用情况
        View::assign('is_use_integral', $is_use_integral);  //优惠券，用able判断是否可用
        View::assign('userCartCouponList', $userCartCouponList);  //优惠券，用able判断是否可用
        View::assign('userCouponNum', $userCouponNum);  //优惠券数量
        ...
        View::assign('cardListNum',count($cartList['cartList']));
        ...
        if(is_weixin()){
            ...
            View::assign('signPackage', $sign_package);
        }
    ...
    public function cart3(){
        ...
        $shopping_card_ids = input('shopping_card_ids/s','');
        ...
        $from_terminal = input("from_terminal/s"); // 下单的终端设备 /H5或微信浏览器端
        ...
        try {
            ...
            $pay->setUserId($this->user_id)->setShopById($shop_id)->delivery($address['district'])->orderPromotion()
                ->useCouponById($coupon_id)->useShoppingCard($shopping_card_ids)->useUserMoney($user_money)->usePayPoints($pay_points,false,'mobile');
            // 提交订单
    ...
    public function integral(){
        ...
        $goods['shop_price'] = $goods['shop_price'] - ($goods['exchange_integral'] / $point_rate);
        ...
        if(IS_AJAX){
           ...
           //B.2发票信息
            ...
            if(empty($info)){
                ...
                (Db::name('user_extend')->where($map)->save($data))?
                $status=1:$status=-1;                
            }            
    ...
    public function shop(){
        $shop_id = input('shop_id/d',false);
        if(!$shop_id){
            $this->error('门店不存在',url('mobile/index/index'));
        }
