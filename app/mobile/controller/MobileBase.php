...
//        Session::start();
        ...
        $first_leader = input('first_leader/d');
        if($first_leader){
        ...
        if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger') && !strstr($_SERVER['HTTP_USER_AGENT'],'miniProgram')
            && !strstr($_SERVER['HTTP_USER_AGENT'],'miniprogram')){
            // 下单的终端设备 微信浏览器
            View::assign('from_terminal', 'mp');

            $this->weixin_config = Db::name('wx_user')->where('1 = 1')->find(); //取微获信配置
            View::assign('wechat_config', $this->weixin_config);            
            $user_temp = session('user');
            if (isset($user_temp['user_id']) && $user_temp['user_id']) {
                $user = Db::name('users')->where("user_id", $user_temp['user_id'])->find();
                if (!$user) {
                    $_SESSION['openid'] = 0;
                    session('user', null);
                }else{
            ...
            if(is_array($this->weixin_config) && $this->weixin_config['wait_access'] == 1 &&  I('oauth') == 'wx' && CONTROLLER_NAME == 'User' && ACTION_NAME == 'binding'){
                $wxuser = $this->GetOpenid(); //授权获取openid以及微信用户信息

                $wxuser['first_leader'] = I('first_leader',0);
                //储存上级关系
                (new UsersLogic())->createRecomRelationship($wxuser);
                //过滤特殊字符串
                ...
                if ($data['status'] != 1 && $data['result'] === '100') {
                    ...
                    return redirect(url('Mobile/User/binding',['first_leader'=>$first_leader,'sss'=>ACTION_NAME]));
                }
                if($data['status'] == 1){
    ...
    {
        if(!session('user')){
            $user = Db::name('users')->where(['user_id'=>input('user_id'),'token'=>input('token')])->find();
            if($user){
                session('user',$user);
            }
        }
    ...
    public function public_assign()
    {
        ...
        View::assign('first_login', $first_login);
       ...
       $brand_list = Db::name('brand')->cache(true,TPSHOP_CACHE_TIME)->field('id,cat_id,logo,is_hot')->where("cat_id > 0")->select();
       View::assign('brand_list', $brand_list);
       View::assign('tpshop_config', $this->tpshop_config);
       /** 修复首次进入微商城不显示用户昵称问题 **/
