...
class Team extends MobileBase
{
    ...
    public function category()
    {
        ...
        if (!$goods_category_level_two->isEmpty()) {
            $goods_category_level_two_arr = get_arr_column($goods_category_level_two, 'id');
            $two_all_ids = implode(',', $goods_category_level_two_arr);
        }
        View::assign('goods_category_level_one', $goods_category_level_one);
        View::assign('goods_category_level_two', $goods_category_level_two);
        View::assign('two_all_ids', $two_all_ids);
        return View::fetch();
    }
    ...
    public function AjaxTeamList()
    {
        ...
        $goods_where = [];
        if ($id && $two_all_ids) {
            $category_three_ids = Db::name('goods_category')->where(['parent_id' => ['in', $two_all_ids]])->column('id');//三级分类id
            $goods_where['cat_id'] = ['in', $category_three_ids];
        }
        if ($tid) {
            $category_three_ids = Db::name('goods_category')->where(['parent_id' => $tid])->column('id');//三级分类id
            $goods_where['cat_id'] = ['in', $category_three_ids];
        }
        $team_where = ['a.status' => 1, 'a.is_lottery' => 0, 'a.deleted' => 0];
        if (count($goods_where) > 0) {
            $goods_ids = Db::name('goods')->where(['is_on_sale' => 1])->where($goods_where)->column('goods_id');
            if (!empty($goods_ids)) {
        ...
        $team_goods_items = $TeamGoodsItem->alias('i')
                ->join('team_activity a', 'a.team_id = i.team_id')
                ->with([ 'goods', 'specGoodsPrice'])
                ->where($team_where)
                ->group('i.goods_id')
                ->order('a.team_id desc')
                ->page($p, 10)
                ->select();
    ...
    public function ajaxTeamFound()
    {
        ...
        $team_ids = $TeamActivity->where(['goods_id' => $goods_id, 'status' => 1, 'is_lottery' => 0, 'deleted' => 0])->column('team_id');
        ...
        if (count($team_ids) > 0) {
            ...
            if (!$teamFounds->isEmpty()) {
                $teamFounds = $teamFounds->append(['surplus'])->toArray();
            }
    ...
    public function addOrder()
    {
        ...
        if ($this->user_id == 0) {
            $this->ajaxReturn(['status' => -101, 'msg' => '购买拼团商品必须先登录', 'result' => ['url' => url('User/login')]]);
        }
        if (empty($goods_num)) {
    ...
    public function order()
    {
        ...
        if (empty($order)) {
            $this->error('订单不存在或者已取消', url("Mobile/Order/order_list"));
        }
        ...
        //检查是拼团订单，查看参团订单的开团人的活动数是否满员，满员则不可支付，则取消订单
        if ($order['prom_type'] == 6 && $order['pay_status'] == 0 )
        {
            $team_follow = Db::name('team_follow')->where('order_id',$order['order_id'])->find();
            if($team_follow && $team_follow['status'] == 0)
            {
                $team_found = Db::name('team_found')->where('found_id',$team_follow['found_id'])->find();
                if($team_found['join'] >= $team_found['need'] )
                {
                    $logic = new \app\common\logic\OrderLogic();
                    $logic->cancel_order($this->user_id, $order['order_id'] );
                    $this->error('拼团活动开团人数已满,订单已取消', url("Mobile/Order/team_list"));
                }
            }
        }
        ...
        if ($order['pay_status'] == 1) {
            $order_detail_url = url("Mobile/Order/order_detail", array('id' => $order_id));
            return redirect($order_detail_url);
        }
        if ($order['order_status'] == 3) {   //订单已经取消
            $this->error('订单已取消', url("Mobile/Order/team_list"));
        }
        //微信浏览器
        ...
        if ($order['coupon_price'] <= 0) {
            ...
            View::assign('userCartCouponList', $userCartCouponList);
            View::assign('order_can_use_coupon_num', $order_can_use_coupon_num);
        }
    ...
    public function getOrderInfo()
    {
        ...
        $act = input('post.act', '');
        if (empty($this->user_id)) {
            $this->ajaxReturn(['status' => 0, 'msg' => '登录超时', 'result' => ['url' => url("User/login")]]);
        }
        if (empty($order_id)) {
        ...
        try {
            ...
            //给h5端使用
            if($error['status'] == -6 ){
                $error['result']['url'] = url('User/paypwd');
                session('payPriorUrl',url('Team/order',array('order_id'=>$order_id)) );
            }
            $this->ajaxReturn($error);
        }
    }
    ...
    public function found()
    {
        ...
        View::assign('teamFollow', $teamFollow);//团员
        View::assign('team', $teamFound->teamActivity);//活动
        View::assign('teamFound', $teamFound);//团长        
        View::assign('userId', $this->user_id);
        return View::fetch();
    }
    ...
    public function ajaxGetMore()
    {
        $p = input('p/d', 0);
        $TeamGoodsItem = new TeamGoodsItem();
        $team_goods_items = $TeamGoodsItem->with(['goods'])->alias('i')->join('team_activity a', 'a.team_id = i.team_id')
        ...
        if (empty($team_goods_items)) {
            ...
            $result = $team_goods_items->append(['team_activity' => ['virtual_sale_num']])->toArray();
    ...
    public function lottery()
    {
        ...
        View::assign('team_lottery', $team_lottery);
        return View::fetch();
    }
