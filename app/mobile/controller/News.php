...
class News extends MobileBase
{
    ...
    public function news_list()
    {
        $news_cat= Db::name('news_cat')
            ...
            ->order('sort_order')
            ->select();
        $cat_id = input('cat_id/d',$news_cat[0]['cat_id']);
        View::assign('news_cat',$news_cat);
    ...
    public function newsComment()
    {
        ...
        if(get_magic_quotes_gpc()){
            ...
            $content=htmlspecialchars(addslashes(input('content')));
        }
        $data=['article_id'=>$article_id,'user_id'=>$this->user_id,'content'=>$content,'add_time'=>time()];
        $res=Db::name('news_comment')->insertGetId($data);
        if($res){
    ...
    public function newsLike()
    {
        ...
        if($like){
            ...
            $res=Db::name('news_like')->insertGetId($data);
            if($res){
                $return=['status' => 1, 'msg' => '点赞成功！'];
            }else{
    ...
    public function newsCollection()
    {
        ...
        if($collection){
            ...
            $res=Db::name('news_collection')->insertGetId($data);
            if($res){
                $return=['status' => 1, 'msg' => '收藏成功！'];
            }else{
