...
class ShopOrder extends Base
{
    ...
    public function index()
    {
        ...
        $where = ['s.shop_id' => $this->shopper['shop_id'], 'o.order_status' => ['>=', 1]];
        ...
        $count = $ShopOrder->alias('s')->join('order o', 's.order_id = o.order_id')->where($where)->count('shop_order_id');
        $Page = new Page($count, 10);
        $list = $ShopOrder->alias('s')->join('order o', 's.order_id = o.order_id')->where($where)->limit($Page->firstRow,$Page->listRows)->order(['s.add_time' => 'desc'])->select();
    ...
    public function off()
    {
        $shop_order_wait_off_num = Db::name('shop_order')->alias('s')->join('order o', 's.order_id = o.order_id')
    ...
    public function statistic()
    {
        ...
        $shop_order_today_count = $ShopOrder->alias('s')->join('order o', 's.order_id = o.order_id')
            ->where([
			[Db::raw("DATE_FORMAT(s.take_time, '%Y-%m-%d')",'=', $now_date], 
			's.shop_id' => $this->shopper['shop_id'], 'o.pay_status' => 1, 'o.order_status' => ['in', [1, 2, 4]]])
            ->count('s.shop_order_id');//今日销售总额
        $shop_order_sum_count = $ShopOrder->alias('s')->join('order o', 's.order_id = o.order_id')
