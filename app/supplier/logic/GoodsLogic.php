...
class GoodsLogic 
{
    ...
    {                  
        $cat = Db::name("GoodsCategory")->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下
        if($cat['parent_id_path'] == '')
        {
            ...
            $cat = Db::name("GoodsCategory")->where("id = $id")->find(); // 从新找出他自己
        }        
        ...
        if($cat['parent_id'] == 0) //有可能是顶级分类 他没有老爸
        {
            ...
            $parent_cat = Db::name("GoodsCategory")->where("id = {$cat['parent_id']}")->find(); // 找出他老爸的parent_id_path
        }        
    ...
    {  
        ...
        if($goods_type == 0)  
        {
            Db::name('GoodsAttr')->where('goods_id = '.$goods_id)->delete(); 
            return;
        }
        
            $GoodsAttrList = Db::name('GoodsAttr')->where('goods_id = '.$goods_id)->select();
               ...
               {                      
                   ...
                   {   
                       ...
                       {                       
                            ...
                            Db::name('GoodsAttr')->where("goods_attr_id = $goods_attr_id")->save(array('attr_price'=>$attr_price));                       
                       }
                   }
                   else // 否则这个属性 数据库中不存在 说明要做删除操作
                   {
                       Db::name('GoodsAttr')->insert(array('goods_id'=>$goods_id,'attr_id'=>$attr_id,'attr_value'=>$v2,'attr_price'=>$attr_price));                       
                   }
            ...
            {                
               Db::name('GoodsAttr')->where('goods_attr_id = '.$v['goods_attr_id'])->delete(); // 
            }                       
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
        ...
        foreach ($spec_arr_sort as $key =>$val)
        {
         ...
         $specItem = Db::name('SpecItem')->column('id,item,spec_id','id');//规格项
         $keySpecGoodsPrice = Db::name('SpecGoodsPrice')->where('goods_id = '.$goods_id)->column('key,key_name,price,store_count,bar_code,sku,cost_price,commission','key');//规格项
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list =  Db::name('goods_category')->column('id,parent_id,level','id');
    ...
    {
        $brandList = config('getSortBrands');
        if(!empty($brandList)){
            return $brandList;
        }
        $brand_where=[];
        ...
        $brandList =  Db::name("Brand")->cache(true)->where($brand_where)->select();
        $brandIdArr =  Db::name("Brand")->cache(true)->where($brand_where)->where("name in (select `name` from `".config('database.connections.mysql.prefix')."brand` group by name having COUNT(id) > 1)")->column('cat_id','id');
        $goodsCategoryArr = Db::name('goodsCategory')->cache(true)->where("level = 1")->column('name','id');
        $nameList = array();
        ...
        array_multisort($nameList,SORT_STRING,SORT_ASC,$brandList->toArray());
    ...
    function getSortCategory()
    {
        ...
        $categoryList =  Db::name("GoodsCategory")->cache(true)->column('id,name,parent_id,level','id');
        $nameList = array();
    ...
    public function afterSave($goods_id)
    {
        ...
        if(count($goods_images) > 1)
        {
            ...
            {
                if(!in_array($val, $goods_images)) Db::name('GoodsImages')->where("img_id = {$key}")->delete();
            }
            // 添加图片
        ...
        {
            Db::name("GoodsImages")->insert(array('goods_id'=>$goods_id,'image_url'=>$original_img));
        }
        delFile(UPLOAD_PATH."goods/thumb/$goods_id"); // 删除缩略图

        // 商品规格价钱处理
        $goods_item = input('item/a'); // 这里没有传market_price
		...
		$supplier = session('supplier');
                update_stock_log($supplier['suppliers_id'],$stock,array('goods_id'=>$goods_id,'goods_name'=>input('goods_name'),'spec_key_name'=>$v['key_name']), '', 1);
                // 修改商品后购物车的商品价格也修改一下
