...
class StockLogic
{
    ...
    {
		$goodsIds = Db::name('goods')->where('suppliers_id', $this->suppliersId)->where('audit', 0)->column('goods_id','goods_id');
		if ($goodsIds) {
    ...
    public function doChangeStockVal()
    {
        ...
        try{
            ...
            if($item_ids){ //对规格商品进行库存修改
                ...
                Db::name('SpecGoodsPrice')->where('item_id','in', $item_ids)->where('goods_id', 'in', $this->goodsIds)->update(['store_count'=>$num]);  //更新spec_goods_price的store_count
                $goods_id = Db::name('SpecGoodsPrice')->where('item_id','in',$item_ids)->where('goods_id', 'in', $this->goodsIds)->field('goods_id')->select(); //找出规格商品对应的goods_id
            ...
            $stockLog = new \app\common\model\StockLog;
