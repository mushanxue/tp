...
{    
    ...
    $arr = array(
        '首页'=>'/',
        $navigate[$location]['name']=>url('/Home/'.CONTROLLER_NAME),
...
function navigate_goods($id, $type = 0)
{
    ...
    // 如果传递过来的是
    if ($type == 1) {
        $cat_id = Db::name('goods')->where("goods_id", $id)->value('cat_id');
    }
    $categoryList = Db::name('GoodsCategory')->column('id,name,parent_id','id');
