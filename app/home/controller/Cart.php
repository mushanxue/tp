...
class Cart extends Base
{
    ...
    public function index()
    {
        ...
        View::assign('userCartGoodsTypeNum', $userCartGoodsTypeNum);
        View::assign('cartList', $cartList);//普通购物车列表
        return View::fetch();
    }
    ...
    public function AsyncUpdateCart()
    {
        $cart = input('cart/a', []);
        $cartLogic = new CartLogic();
        $cartLogic->updateCartPrice($this->user);
        $cartLogic->setUserId($this->user_id);
    ...
    public function getStoreCoupon()
    {
        ...
        if ($newStoreCoupon) {
            $user_coupon = Db::name('coupon_list')->where('uid', $this->user_id)->column('cid');
    ...
    public function cart2()
    {
        ...
        if ($this->user_id == 0) {
            session('redirect_url',url('Home/Goods/goodsInfo',['id'=>$goods_id]));
        ...
        $card_data['goods_category_id']=$cartList['cartList'][0]['cat_id'];
        $card_data['goods_id']=$cartList['cartList'][0]['goods_id'];

        $logic = new ShoppingCardLogic();
        $userCardNum= $logic->getUsableNum($card_data);
        View::assign('userCardNum',$userCardNum);;

        $card_data['usable']=1;
        $usableCard = $logic->getMyCard($card_data)['card'];
        $card_data['usable']=2;
        $unsableCard = $logic->getMyCard($card_data)['card'];
        View::assign('usableCard',$usableCard);
        View::assign('unsableCard',$unsableCard);
        ...
        View::assign('userCartCouponList', $userCartCouponList);  //优惠券，用able判断是否可用
        View::assign('cartGoodsTotalNum', $cartGoodsTotalNum);
        View::assign('cartList', $cartList['cartList']); // 购物车的商品
//        halt($cartList['cartList'][0]['combination_cart']);
        View::assign('cartPriceInfo', $cartPriceInfo);//商品优惠总价
        return View::fetch();
    }
    ...
    {
        $address_list = Db::name('UserAddress')->where(['user_id' => $this->user_id])->order('is_default desc')->select()->toArray();
        if ($address_list) {
            $area_id = array();
            ...
            $regionList = Db::name('region')->where("id", "in", $area_id)->column('name','id');
    ...
    public function cart3()
    {
        ...
        $data = input('request.');
        $shopping_card_ids = input('shopping_card_ids/s','');//使用的购物卡
        $from_terminal = input("from_terminal/s"); // 下单的终端设备 /PC端
        ...
        try {
            ...
            $pay->setUserId($this->user_id)
                ...
                ->useShoppingCard($shopping_card_ids)
    ...
    public function cart4()
    {
        ...
        if ($order['pay_status'] == 1) {
            if($order['prom_type']==10){
                $order_detail_url = url("Home/ShoppingCard/MyCard");
            }else{
                $order_detail_url = url("Home/Order/order_detail", array('id' => $order['order_id']));
            }
    ...
    public function header_cart_list()
    {
        ...
        View::assign('cartList', $cartList); // 购物车的商品
        View::assign('cartPriceInfo', $cartPriceInfo); // 总计
    ...
    public function buyIntegralGoods()
    {
        ...
        try {
            $Integral->checkBuy();
            $url = url('Cart/integral', ['goods_id' => $goods_id, 'item_id' => $item_id, 'goods_num' => $goods_num]);
    ...
    public function save_invoice()
    {
        if (IS_AJAX) {
            ...
            //B.1校验用户是否有历史发票记录
            $map['user_id'] = $this->user_id;
            $info = Db::name('user_extend')->where($map)->find();
            //B.2发票信息
            $data = [];
            ...
            if (empty($info)) {
                ...
                Db::name('user_extend')->insert($data)? $status=1:$status=-1;
            }else{
                (Db::name('user_extend')->where($map)->save($data)) === false ? $status = -1 : $status = 1;
            }
    ...
    public function pre_sell()
    {
        ...
        View::assign('cartList', $cartList);//购物车列表
        View::assign('preSell', $preSell);
        View::assign('cartTotalPrice', $cartTotalPrice);
        return View::fetch();
    }
    ...
    public function pre_sell_place()
    {
        ...
        $pay = new Pay();
        $PreSell = new PreSell();
        $preSell = $PreSell::find($pre_sell_id);
        $PreSellLogic = new PreSellLogic($preSell->goods, $preSell->specGoodsPrice);
        try{
