...
class Api extends Base
{
    ...
    public function queryExpress()
    {
        ...
        if ($express_switch == 1) {
            ...
            $data['LogisticCode'] = trim(input('invoice_no'));
            // 顺丰的有可能需要手机后四位
    ...
    public function check_order_pay_status()
    {
        ...
        $recharge = input('recharge/d');
        if ($recharge == 1) {
            // 充值查询
            $order = Db::name('recharge')->field('pay_status')->where(['order_id' => $order_id])->find();
    ...
    public function ad_show()
    {
        ...
        $where = array(
            'pid' => $pid,
            'enable' => 1,
            'start_time' => array('<', strtotime(date('Y-m-d H:00:00'))),
            'end_time' => array('>', strtotime(date('Y-m-d H:00:00'))),
        );
        $ad = Db::name("ad")->where($where)->order("orderby desc")->cache(true, TPSHOP_CACHE_TIME)->find();
        View::assign('ad', $ad);
        return View::fetch();
    }
...
//        $ip = '183.147.30.238';//测试ip
    ...
    public function shop()
    {
        ...
        if (!$shop_list->isEmpty()) {
            $shop_list = $shop_list->append(['phone','area_list','work_time','work_day'])->toArray();
    ...
    public function add_visit(){
        ...
        $portrait->add_visit($user,$type,$goods_id);
    }
    /**
     * 自定义底部菜单数据
     */
