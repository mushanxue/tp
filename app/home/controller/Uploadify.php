...
class Uploadify extends Base {
	...
	*/
	...
	public function fileList(){
		...
		$path = UPLOAD_PATH.'user/'.cookie('user_id').'/'.input('path','temp');
		if(strstr($path,'../'))
			exit;		
		$listSize = 100000;
	...
	*/
	...
	public function imageUp()
	{
		...
		if(empty($file) && ($_FILES['upfile']['error'] != 4))
			$file = request()->file('upfile');
                
                if(empty($file))
                {
                    $return_data['title'] = '文件上传';                    
                    $return_data['state'] = '上传文件图片失败';
		...
		if ($state || !$file) {
			...
			$ossSupportPath = ['comment', 'photo','head_pic'];
			if (in_array(input('savepath'), $ossSupportPath) && $ossConfig['oss_switch']) {
				//商品图片可选择存放在oss
				$object = UPLOAD_PATH.$savePath.'/'.md5(time()).'.'.pathinfo($file->getOriginalName(), PATHINFO_EXTENSION);
				$ossClient = new \app\common\logic\OssLogic;
				$return_url = $ossClient->uploadFile($file->getPathName(), $object);
				if (!$return_url) {
