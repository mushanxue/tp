...
class Api extends Base {
    ...
    public function getRegion(){
        ...
        $data = Db::name('region')->where("parent_id", $parent_id)->cache(true)->select();
        $html = '';
        if($data){
            foreach($data as $h){
    ...
    public function getSpecPrice(){
        ...
        $goods = Db::name('spec_goods_price')->where(array('key'=>$key,'goods_id'=>$goods_id))->find();
        $info = array(
            'status' => 1,
            'msg' => 0,
    ...
    public function calcGoods(){
        $goods_id = input('post.goods/d'); // 添加商品id
        $price_type = input('post.price') ? input('post.price') : 3; // 价钱类型
        ...
        switch($price_type){
            ...
            case 2:
                ...
                break;
            case 3:
                $goods_price = input('post.goods_price'); //自定义
                break;
        }

        $goods_num = input('post.goods_num/d'); // 商品数量
