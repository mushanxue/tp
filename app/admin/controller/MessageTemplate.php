...
class MessageTemplate extends Base {
    ...
    public function editTemplate(){
        ...
        {  
            $data = input('post.');
            $userMsgTpl = validate(\app\admin\validate\UserMsgTpl::class);

            if (!$userMsgTpl->batch(true)->scene($data['act'])->check($data)) {
                ...
                $return_arr = array('status' => 1,'arr'=>$arr, 'msg' => '操作成功', 'url' => url('Admin/MessageTemplate/index'));
            }

            return $this->ajaxReturn($return_arr);
        }

        $mmt_code = input('mmt_code');
        if (!empty($mmt_code)) {
            $arr = Db::name("user_msg_tpl")->where('mmt_code', $mmt_code)->find();
            View::assign('arr', $arr);
        }
    ...
    public function delTemplate(){
        ...
        $row = Db::name("user_msg_tpl")->where('mmt_code', input('id'))->delete();
        $return_arr = array();
        if ($row){
