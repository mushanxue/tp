...
class Template extends Base {
    ...
    public function templateList()
    {
        ...
        View::assign('t', $t);
        // $default_theme =  tpCache("hidden.{$t}_default_theme"); // //$default_theme = Db::name('Config')->where("name='{$t}_default_theme'")->value('value');
        $template_arr = include(APP_PATH."/$m/html.php");
        View::assign('default_theme', $template_arr['template']['default_theme']);
...
?>";
        ...
        delFile('./runtime');
        $this->success("操作成功!!!",url('admin/template/change',array('t'=>$t)));
    }
    ...
    public function block_item_add(){
    	$model_mb_special = (new \app\common\model\MbSpecial());
    ...
    public function block_item_edit(){
    	...
    	$item_info = $model_mb_special->getMbSpecialItemInfoByIDb::name($_GET['item_id']);
    ...
    public function block_item_saveOp() {
    	...
    	$result = $model_mb_special->editMbSpecialItemByIDb::name(array('item_data' => $_POST['item_data']), $_POST['item_id'], $_POST['special_id']);
