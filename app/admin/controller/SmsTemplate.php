...
class SmsTemplate extends Base {
    ...
    public function __construct() {
        ...
        View::assign('send_scene', $send_scene);  
        
    }
    
    public function index(){
        
        $smsTpls = Db::name('sms_template')->select();
		View::assign('smsTplList',$smsTpls);
		View::assign('k','list');
        return View::fetch("sms_template_list");
    ...
    public function sms_log(){
        ...
        View::assign('smsLogList',$smsLogList);
        View::assign('page',$show);// 赋值分页输出
    	View::assign('pager',$page);
        
        return View::fetch("sms_log");
    ...
    public  function addEditSmsTemplate(){
        ...
        {    
            ...
            //exit;
            if($id){
               Db::name("sms_template")->update($data);
            }else{
                $id = Db::name("sms_template")->save($data);
            }
            $this->success("操作成功!!!",url('Admin/SmsTemplate/index'));
            exit;
        } 
        ...
        if($id){
            ...
            View::assign("smsTpl" , $smsTemplate );
            $sceneName = $this->send_scene[$smsTemplate['send_scene']][0];
            $sendscene = $smsTemplate['send_scene'];
            View::assign("send_name" , $sceneName );
            View::assign("send_scene_id" , $sendscene );
        }else{
            //进入添加页面
            //查找已经添加了的短信模板
            $scenes = $model->column("send_scene" );
        ...
        View::assign("send_scene" , $filterSendscene );
        return View::fetch("_sms_template");
    }
   ...
   public function delTemplate(){
       ...
       $row = Db::name("sms_template")->where('tpl_id ='.$_GET['id'])->delete();
       $return_arr = array();
       if ($row){
