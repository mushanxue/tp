...
exit("请联系TPshop官网客服购买高级版支持此功能");
    ...
    public function off()
    {
        $shop_order_wait_off_num = Db::name('shop_order')->alias('s')
            ->join('order o','o.order_id = s.order_id')->where(['s.is_write_off' => 0,'order_status'=>1,'o.prom_type'=>['=',9]])->count('s.shop_order_id');
    ...
    public function statistic()
    {
        ...
        $where['o.prom_type']= ['=',9]; //自提订单prom_type为9
        $Shop = new Shop();
        ...
        $shop_order_today_count = $ShopOrder->alias('s')->join('order o','s.order_id = o.order_id')
			...
			[Db::raw("DATE_FORMAT(FROM_UNIXTIME(s.add_time,'%Y-%m-%d'), '%Y-%m-%d')"),'=',$now_date],
			 'o.pay_status' => 1, 'o.order_status' => ['in', [1, 2, 4]],'o.prom_type'=> ['=',9]])->count();//今日销售总额
    ...
    public function export_order()
    {
        ...
        $where['o.prom_type'] = ['=',9];//自提订单为9
        $ShopOrder = new \app\common\model\ShopOrder();
        $orderList = $ShopOrder->alias('s')->join('order o', 's.order_id = o.order_id')->where($where)->order('o.order_id')->select();
