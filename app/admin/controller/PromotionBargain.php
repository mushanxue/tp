...
exit("请联系TPshop官网客服购买高级版支持此功能");
    ...
    {
        $id = input('del_id/d');
        $bargain_first = Db::name('bargain_first')->where(['bargain_id'=>$id,'is_end'=>0])->find();
        if($bargain_first){ $this->ajaxReturn(['status'=>0,'msg'=>'该活动已存在订单，不能删除']);}
        if ($id) {
            $PromotionBargain = \app\common\model\PromotionBargain::where(['id'=>$id])->find();
            if($PromotionBargain['promotion_bargain_goods_item'][0]['item_id'] > 0){
                ...
                Db::name('spec_goods_price')->where(['item_id'=>['IN', $item_ids],'prom_id'=>$id,'prom_type'=>8])->save(['prom_type' => 0, 'prom_id' => 0]);
                $goodsPromCount = Db::name('spec_goods_price')->where(['goods_id'=>$PromotionBargain['goods_id']])->where('prom_type','>',0)->count('item_id');
                if($goodsPromCount == 0){
                    Db::name('goods')->where("goods_id", $PromotionBargain['goods_id'])->save(['prom_type' => 0, 'prom_id' => 0]);
                }
            }else{
                Db::name('goods')->where(["goods_id"=>$PromotionBargain['goods_id'], 'prom_id' => $id])->save(['prom_type' => 0, 'prom_id' => 0]);
            }
...
//            }
            ...
            Db::name('promotion_bargain_goods_item')->where(['bargain_id' => $id])->update(['deleted'=>1]);
            // 删除砍价消息
            $messageFactory = new MessageFactory();
