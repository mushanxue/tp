...
class BeSpeakOrder extends Base {
    ...
    public function index(){
        ...
        $where['prom_type'] = ['=',7];  //预约订单类型为7
        //$where['s.goods_id'] = ['>',0];  //区分自提订单
        $ShopOrder = new \app\common\model\ShopOrder();

        $count = $ShopOrder->alias('s')->join('order o', 's.order_id = o.order_id')->where($where)->group('s.order_id')->count('s.shop_order_id');
        $Page = new Page($count, 10);
        $list = $ShopOrder->alias('s')->join('order o', 's.order_id = o.order_id')->where($where)->limit($Page->firstRow,$Page->listRows)->order(['o.add_time'=>'desc'])->group('s.order_id')->select();
        $Shop = new Shop();
        ...
        View::assign('take_time', $take_time);
        View::assign('add_time_start', $add_time_start);
        View::assign('add_time_end', $add_time_end);
        return View::fetch();
        /*code_16门店管理模块代码*/
    }
    ...
    public function off()
    {
        $shop_order_wait_off_num = Db::name('shop_order')->alias('s')
            ->join('order o','o.order_id = s.order_id')->where(['s.is_write_off' => 0,'s.goods_id' =>['>',0],'o.order_status'=>1,'o.prom_type'=>7])->count('s.shop_order_id');
        View::assign('shop_order_wait_off_num', $shop_order_wait_off_num);
        return View::fetch();
    }
    /**
     * ajax 获取预约订单信息
...
//            $where['s.take_time'] = ['between', [date('Y-m-d',$take_time_start), date('Y-m-d',$take_time_end)]];
        ...
        $where['o.prom_type']= ['=',7]; //7为预约订单
        $Shop = new Shop();
        $ShopOrder = new \app\common\model\ShopOrder();
        $now_date = date('Y-m-d');
        $shop_list = $Shop->where(['deleted'=>0])->cache(true)->select();
        $shop_order_today_count = $ShopOrder->alias('s')->join('order o','s.order_id = o.order_id')
            ->where([
			[Db::raw("DATE_FORMAT(FROM_UNIXTIME(s.add_time,'%Y-%m-%d'), '%Y-%m-%d')"),'=', $now_date],
			's.goods_id' =>['>',0], 
			'o.pay_status' => 1, 
			'o.order_status' => ['in', [1, 2, 4]]
			])->group("s.order_id")->count('s.order_id');//今日销售总额
        //COUNT(DISTINCT s.order_id) DISTINCT 函数返回指定列的不同值的数目：类似group分组
        $shop_order_sum_list = $ShopOrder->alias('s')->join('order o','s.order_id = o.order_id')
        ...
        View::assign('shop_list', $shop_list);
        View::assign('take_time_start', $take_time_start);
        View::assign('take_time_end', $take_time_end);
        View::assign('shop_order_today_count', $shop_order_today_count);
        View::assign('shop_order_sum_list', $shop_order_sum_list);
        //开始拼装数组
        ...
        View::assign('table', json_encode($table));
        return View::fetch();
    }
    ...
    public function shop_info(){
        ...
        View::assign('shop_order',$shop_order);
        $BespeakTemplateUnit =  new BespeakTemplateUnit();
        //获取表信息
        $order_bespeak = $BespeakTemplateUnit::hasWhere('OrderBespeak',['order_id'=>$order_id])->field('OrderBespeak.*')->order('sort desc')->select();
...
//        }
        ...
        $ShopOrder = new \app\common\model\ShopOrder();
        $orderList = $ShopOrder->alias('s')->join('order o', 's.order_id = o.order_id')->where($where)->order(['o.add_time'=>'desc'])->group('s.order_id')->select();
