...
class Miniapp extends Base
{
	...
	{
		$miniapp = MiniappModel::where(['user_id' => $this->appService['user_id'], 'miniapp_id' => $this->appService['miniapp_id']])->find();
	...
	public function release_manage(){
		...
		View::assign('miniapp', $miniapp);
		View::assign('test_user_miniApp', $test_user_miniApp);
		View::assign('audit_user_miniApp', $audit_user_miniApp);
		View::assign('release_user_miniApp', $release_user_miniApp);
		return View::fetch();
	}
	...
	public function audit()
	{
		...
		//废弃审核失败的
		if ($userMiniapp = UserMiniapp::where(['user_id' => $this->appService['user_id'], 'status' => UserMiniapp::STATUS_AUDIT_FAIL])->find()) {
		    $userMiniapp->save(['status' => UserMiniapp::STATUS_ABANDON]);//废弃
		}

// 		if (!UserMiniapp::where(['user_id' => $this->appService['user_id'], 'status' => UserMiniapp::STATUS_TEST])->find()) {
...
// 		}
	...
	{
		$mini_app = MiniappModel::where(['user_id' => $this->appService['user_id'], 'miniapp_id' => $this->appService['miniapp_id']])->find();
	...
	{
		$data = input('post.');
		$validate = validate(\app\admin\validate\MiniAppAudit::class);
	...
	public function auditBack(){
	    ...
	    try{
	        $rs = $miniLogic->back();
	         if($rs){
	         	//废弃审核撤回的
        		if ($userMiniapp = UserMiniapp::where(['user_id' => $this->appService['user_id'], 'status' => UserMiniapp::STATUS_AUDITING])->find()) {
        		    $userMiniapp->save(['status' => UserMiniapp::STATUS_ABANDON]);//废弃
        		}
	         }
