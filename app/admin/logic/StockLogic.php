...
class StockLogic
{
    ...
    public function getStockList()
    {
        ...
        if($ctime){
            $gap = explode(' - ', $ctime);
            $map['ctime'] = array(array('>',strtotime($gap[0])),array('<',strtotime($gap[1])));
        }
    ...
    public function getAjaxLowStockWarn()
    {
        ...
        $map['store_count'] = array('<', $warning_storage);
        //获取无规格的的商品库存小于预警值的数量
        $table = config('database.connections.mysql.prefix').'spec_goods_price';  //解决__PREFIX__解析错误
        $num1 = Db::name('Goods')->alias('a')
            ->whereNotExists("select `goods_id` FROM {$table} WHERE `goods_id`= a.goods_id")
            ->where($map)->count();
        //查询无规格的库存小于预警值的商品信息
        $sql = Db::name('Goods')->alias('a')
            ->whereNotExists("select `goods_id` FROM {$table} WHERE `goods_id`= a.goods_id")
        ...
        $catList = Db::name('goods_category')->field('id,name')->select();
        $brandList = Db::name('brand')->field('id,name')->select();
    ...
    public function getAjaxAlterStock()
    {
        ...
        $map = $this->get_map();
        $listRows = input('listRows') ? input('listRows') : 20;
        //获取无规格的的商品数量
        $table = config('database.connections.mysql.prefix').'spec_goods_price';  //解决__PREFIX__解析错误
        $num1 = Db::name('Goods')->alias('a')
            ->whereNotExists("select `goods_id` from {$table} WHERE `goods_id`= a.goods_id")
    ...
    public function doChangeStockVal($admin_id)
    {
        ...
        try{
            ...
            if($item_ids){ //对规格商品进行库存修改
                ...
                $spec_info = Db::name('SpecGoodsPrice')->alias('a')
                ...
                Db::name('SpecGoodsPrice')->where('item_id','in', $item_ids)->update(['store_count'=>$num]);  //更新spec_goods_price的store_count
                $goods_id = Db::name('SpecGoodsPrice')->where('item_id','in',$item_ids)->field('goods_id')->select(); //找出规格商品对应的goods_id
                $goods_id = array_unique(get_arr_column($goods_id,'goods_id')); //找出需要修改的规格商品的goods_id
                $data = array();
                foreach ($goods_id as $value) //获取规格商品的总库存
                {
                    $count = Db::name('SpecGoodsPrice')->where('goods_id',$value)->sum('store_count');
                    $data[] = array('store_count'=>$count,'goods_id'=>$value);
                }
                
                $goods_obj = new \app\common\model\Goods;                
                $goods_obj->saveAll($data);
                //Db::name('Goods')->saveAll($data); //更新规格商品的总库存
                
            }
          
            if($goods_ids){ //对无规格商品进行库存修改
                $goods_info = Db::name('Goods')->where('goods_id','in', implode(',',$goods_ids))->field('goods_id, store_count, goods_name')->select();//获取原先库存量
                ...
                Db::name('Goods')->where('goods_id','in', implode(',',$goods_ids))->update(['store_count'=>$num]); //更新库存
            }
            ...
            $stockLog_obj = new \app\common\model\StockLog;                
            $stockLog_obj->saveAll($stock_log);            
            //Db::name('stock_log')->saveAll($stock_log);
    ...
    public function get_map(){
        ...
        if(!empty($brand_id)){
            $map['brand_id'] = array('=', $brand_id);
        }
        if('' !==$is_on_sale){
            $map['is_on_sale'] = array('=', $is_on_sale);
        }
        if(!empty($key_word ))
        {
    ...
    */
    public function excel_import($file,$images='')
    {
        ...
        foreach ($arr as $k => $v) {
            //判断商品是否存在
            $result = Db::name('goods')->where('goods_id', $v[0])->find();
            if(!$result){
                return ['msg'=>$v[0].'商品不存在','status'=>0];
            }
            //判断规格是否存在
            if ($v[3] > 0) {
                $result2 = Db::name('spec_goods_price')->where('item_id', $v[3])->find();
                if (!$result2) {
    ...
    public function doExcelChangeStockVal($admin_id)
    {
        ...
        try{
            foreach($data['good'] as $k => $v) {
                ...
                //获取原库存
                if($v['item_id']) {//有规格
                    $info = Db::name('spec_goods_price')->alias('a')
                        ->where('a.item_id',$v['item_id'])->join('Goods g', 'a.goods_id=g.goods_id')
                        ->field('a.goods_id, a.store_count, a.key_name, g.goods_name')
                        ->find();
                } else {//无规格
                    $info = Db::name('goods')->where('goods_id',$v['goods_id'])->field('goods_id, store_count, goods_name')->find();
                }
                ...
                //更新库存
                if($v['item_id']) {//有规格
                    Db::name('spec_goods_price')->update(['item_id'=>$v['item_id'], 'store_count'=>$v['store_count']]);//改变规格库存
                    $count = Db::name('spec_goods_price')->where('goods_id', $v['goods_id'])->sum('store_count');
                    Db::name('goods')->update(['goods_id'=>$v['goods_id'], 'store_count'=>$count]);//更新规格商品的总库存
                } else {//无规格
                    Db::name('goods')->update(['goods_id'=>$v['goods_id'], 'store_count'=>$v['store_count']]);//更新规格商品的总库存
                }
            }
