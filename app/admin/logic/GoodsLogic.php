...
class GoodsLogic 
{
    ...
    function getSortBrands($cat_id=0)
    {
        ...
        $brandList =  Db::name("Brand")->cache(true)->where($brand_where)->select()->toArray();
    ...
    public function afterSave($goods_id)
    {
        ...
        $num = Db::name('GoodsImages')->where("goods_id = $goods_id ")->count();
        ...
        $deal_orignal_img= trim($deal_orignal_img,'/');
        if($c == 0  && $num== 0  && $original_img && file_exists($deal_orignal_img)) //@}
        {
        ...
        if ($goods_item) {
            ...
            foreach ($goods_item as $k => $v) {
                ...
                //记录库存日志
                update_stock_log(session('admin_id'),$stock,array('goods_id'=>$goods_id,'goods_name'=>input('goods_name'),'spec_key_name'=>$v['key_name']));
                // 修改商品后购物车的商品价格也修改一下
