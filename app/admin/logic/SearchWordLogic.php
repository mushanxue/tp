...
class SearchWordLogic 
{
    public function initGoodsSearchWord(){
        ...
        Db::execute('TRUNCATE table '.config('database.connections.mysql.prefix').'search_word');
        ...
        foreach($goodsKeyWords as $key=>$val){
            ...
            $success = Db::name('search_word')->insertGetId($search_add_data);
            if($success !== false){
                $successNum++;
            }
    ...
    public function getSearchWordWhere($q){
        ...
        if (file_exists(PLUGIN_PATH . 'coreseek/sphinxapi.php')) {
            ...
            $cl = new \SphinxClient();
            $cl->SetServer(config('SPHINX_HOST') . '', intval(config('SPHINX_PORT')));
