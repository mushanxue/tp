...
class Combination extends Validate
{
    ...
    protected function checkCombinationGoods($value, $rule, $data)
    {
        ...
        if($data['combination_id'] > 0){
            $is_master_goods['a.combination_id'] = ['<>',$data['combination_id']];
        }
        $combination_goods_list = Db::name('combination_goods')->alias('a')->join('combination_goods b', 'b.combination_id = a.combination_id')->field('b.*')->where($is_master_goods)->select();
