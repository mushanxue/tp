...
class ArticleCategory extends Validate
{
    ...
    protected function checkEmpty($value,$rule, $data)
    {
        ...
        $name_num = Db::name('article_cat')->where(['cat_name'=>$value])->whereNotIn('cat_id',$data['cat_id'])->count();
        if($name_num > 0){
            return '分类名不能重复';
        }
        return true;
    ...
    protected function checkEdit($value, $rule, $data)
    {
        ...
        //错误时跳转到：url('Admin/Article/category',array('cat_id'=>$data['cat_id']));
    ...
    protected function checkDel($value)
    {
        ...
        $res = Db::name('article_cat')->where('parent_id', $value)->select();
        if (!$res->isEmpty()) {
            return '还有子分类，不能删除';
        }
        
        $res = Db::name('article')->where('cat_id', $value)->select();
        if (!$res->isEmpty()) {
            return '非空的分类不允许删除';
        }
