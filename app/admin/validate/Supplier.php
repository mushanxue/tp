...
class Supplier extends Validate
{
    ...
    protected $rule = [
		...
		'suppliers_account'         => 'require|checkSuppliersAccount',//'require|unique:suppliers',
    ...
    protected function checkUserName($value, $rule, $data)
    {
        ...
        $user_id = Db::name('users')->where($where)->value('user_id');
        if ($user_id) {
            $userSupplierCount = Db::name('suppliers')->where(['user_id' => $user_id,'deleted'=>0])->count();
    ...
    protected function checkSuppliersName($value, $rule, $data)
    {
        ...
        $count=Db::name('suppliers')->where(['suppliers_name'=>$value,'deleted'=>0])->count();
        if($count){
            return '已有相同的供应商名称';
        }
    ...
    protected function checkSuppliersAccount($value,$rule,$data){
        $count=Db::name('suppliers')->where(['suppliers_account'=>$value,'deleted'=>0])->count();
        if($count){
            return '已有相同的供应商账号';
        }else{
            return true;
