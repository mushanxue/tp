...
class PromotionBargain extends Validate
{
    ...
    protected function checkTitle($value, $rule ,$data)
    {
        ...
        $find = Db::name('promotion_bargain')->where(['title'=>$value,'deleted'=>0,'id'=>['<>',$data['id']]])->value('title');
        if($find){
            return '已存在相同抢购标题';
        }
        return true;
...
//        'goods_num.gt'          => '抢购数量必须是大于0的数字',
    ...
    protected function checkPrice($value, $rule ,$data)
    {
        if($data['item_id'] > 0){
            //商品规格
            $price = Db::name("spec_goods_price")->where(['item_id'=>$data['item_id']])->value('price');
        }else{
            $price = Db::name('goods')->where('goods_id',$data['goods_id'])->value('shop_price');
        }
